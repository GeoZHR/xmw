...
def main(args):
  goDisplay()
def goDisplay():
  p1 = readImageM(n1,n2,p1file)
  p2 = readImageM(n1,n2,p2file)
  fs = readImageM(n1,n2,fsfile)
  ss = readImageM(n1,n2,ssfile)
  lof = LocalOrientFilterP(8,8)
  u1 = zerofloat(n1,n2)
  u2 = zerofloat(n1,n2)
  lof.applyForNormal(fs,u1,u2)
  au = fillfloat(0.001,n1,n2)
  av = fillfloat(1.000,n1,n2)
  v2 = mul(-1,p1)
  v1 = p2
  ss1 = zerofloat(n1,n2)
  ss2 = zerofloat(n1,n2)
  ets1 = EigenTensors2(v1,v2,au,av)
  ets2 = EigenTensors2(u1,u2,au,av)
  lsf = LocalSmoothingFilter()
  lsf.apply(ets1,40,ss,ss1)
  lsf.apply(ets2,40,ss,ss2)
  #plot(fs,cmin=min(fs)/4,cmax=max(fs)/4)
  plot(ss,cmin=min(ss)/4,cmax=max(ss)/4)
  plot(ss1,cmin=min(ss1)/4,cmax=max(ss1)/4)
  plot(ss2,cmin=min(ss2)/4,cmax=max(ss2)/4)
  plot(ss,v1=p1,v2=p2,cmin=min(ss)/4,cmax=max(ss)/4)
  plot(ss,v1=mul(-1,u2),v2=u1,cmin=min(ss)/4,cmax=max(ss)/4)
...
def plot(f,g=None,v1=None,v2=None,cmap=None,cmin=None,cmax=None,cint=None,
  ...
  panel.setVLabel("Crossline (samples)")
  ...
  frame.setSize(round(n2*1.1),round(n1*1.1)) #for f3d
