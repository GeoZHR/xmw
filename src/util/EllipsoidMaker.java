 ...
 * @version 2015.04.13
 */
public class EllipsoidMaker {

  public void applyForOneEllipsoid(
    float[] cx, float[] ux, float[] vx, float[] wx) {
    show(new DefineOneEllipsoid(cx,ux,vx,wx));
  }
  ...
  */
  ...
  private static void show(Node node) {
    StateSet states = StateSet.forTwoSidedShinySurface(Color.YELLOW);
    node.setStates(states);
    World world = new World();
    world.addChild(node);
    SimpleFrame sf = new SimpleFrame(world);
    String pngDir = "../../../png/ipfx/slides/";
    String png = "ellpsoid";
  ...
  // One ellipsoid with specified thickness.
  public static class DefineOneEllipsoid extends Node {
    public DefineOneEllipsoid(
      float[] cx,float[] ux, float[] vx, float[] wx) {
      _cx = cx;
      _ux = ux;
      _vx = vx;
