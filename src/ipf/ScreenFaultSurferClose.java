...
public class ScreenFaultSurferClose {
    ...
    FaultCell[] fc, float[][][] wp, float[][][] x, float[][][] y) 
  {
    ...
    for (int ic=0; ic<nc; ++ic) {
      ...
      y[i3m][i2m][i1] += wpi*wpi*x[i3m][i2m][i1]; //screen points with 0.0
      y[i3p][i2p][i1] += wpi*wpi*x[i3p][i2p][i1]; //screen points with 0.0
      y[i3 ][i2 ][i1] += wpi*wpi*x[i3 ][i2 ][i1]; //screen points with -0.5
    }
  }
