...
public class KdTreeM {
  ...
  public interface Distance {
    ...
    public float f2(float fdsum);

    public float fd(int i, float ai, float ui, float bi);
  }
   ...
   */
  public KdTreeM(float[][] x, float[][] u) {
    this(x,u,new EuclideanDistance());
  }
   ...
   */
  public KdTreeM(float[][] x, float[][] u, Distance d) {
    _n = x[0].length;
    _k = x.length;
    _x = x;
    _u = u;
  ...
  private float[][] _u; // array[k][n] of normal vector components
  ...
  private float fdsum(int i, float[] x) {
    float ds = 0.0f;
    for (int j=0; j<_k; ++j)
      ds += _d.fd(j,_x[j][i],_u[j][i],x[j]);
    return abs(ds);
  }
