...
public class FakeData2 {
  ...
  public float[][] getTrainDataAndLabels(float[][][] sx, float[][][] ex) {
    final int n3 = sx.length;
    final int n2 = sx[0].length;
    final int n1 = sx[0][0].length;
    final int m1 = 300;
    final int m2 = 200;
    final int f1 = round((m1-n1)*0.5f);
    final int f2 = round((m2-n2)*0.5f);
  ...
  private int _na,_nb,_nc,_ns,_nn,_nt,_nd;
  private float[] _ax,_bx,_cx,_sx,_nx,_tx,_dx;
