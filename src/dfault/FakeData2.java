...
public class FakeData2 {
  ...
  public FakeData2(float fd, float dd, float ld) {
    ...
    _hx=new float[]{-12,0,-10,-8,0-6,-4,0,-2,-1,0,1,2,0,4,6,0,8,10,0,12};
    _nh = _hx.length;
    _nt = _tx.length;
    _na = _ax.length;
    _nb = _bx.length;
  ...
  /**
   * Compute a random batch of 2D seismics image and label images.
   * @param sx a batch (100) of seismic images to be comptued.
   */
  public float[][] getTrainDataAndLabels(float[][][] sx) {
    ...
    public void compute(int i3) {
      ...
      Random r = new Random();
      int ia = r.nextInt(_na); //parameter a of folding
      int ib = r.nextInt(_nb); //parameter b of folding
      int ic = r.nextInt(_nc); //parameter c of folding
      int is = r.nextInt(_ns); //vertical shearing
      int id = r.nextInt(_nd); //fault dip
      int it = r.nextInt(_nt); //fault throw
      int ih = r.nextInt(_nh); //fault shift
      int in = r.nextInt(_nn); //noise
      float axi = _ax[ia];
      float bxi = _bx[ib];
      float cxi = _cx[ic];
      float sxi = _sx[is];
  ...
  public float[][][] setTestImage(int d, int m1, int m2, float[][] gx) {
    int n2 = gx.length;
    int n1 = gx[0].length;
  ...
  public float[][][] faultStack(
    int n1, int n2,
    float[][] seeds, float[] ft, float[] fl, float[][][] fb) {
    int m2 = fb[0].length;
    int m1 = fb[0][0].length;
    int h2 = round(m2*0.5f)-1;
    int h1 = round(m1*0.5f)-1;
    ...
    for (int ip=0; ip<np; ip++) {
      ...
      if(i2<5||i2>=n2-5) continue;
      float fm = fl[ip];
      int dk = round(ft[ip]);
      System.out.println("fm="+fm);
      if(dk==_nd-1) continue;
      tt[i2][i1] = -_dx[dk];
      int b2 = i2-h2;
      int b1 = i1-h1;
      int e2 = i2+h2;
      ...
      for (int k1=b1; k1<=e1; k1++) {
        fs[k2][k1] += fm*fb[dk][h2+k2-i2][h1+k1-i1];
      }}
    }
    return new float[][][]{fs,tt};
  }

  public float[][][] faultStack(float[][] ft, float[][] fl, float[][][] fb) {
    int n2 = fl.length;
    int n1 = fl[0].length;
    int m2 = fb[0].length;
    int m1 = fb[0][0].length;
    ...
    for (int i2=0; i2<5; i2++)
      zero(fl[i2]);
    for (int i2=n2-5; i2<n2; i2++)
      zero(fl[i2]);
    for (int i2=5; i2<n2-5; i2++) {
    for (int i1=0; i1<n1; i1++) {
      float fm = fl[i2][i1];
      int dk = round(ft[i2][i1]);
      if(dk==_nd-1) continue;
      tt[i2][i1] = -_dx[dk];
      int b2 = i2-h2;
      int b1 = i1-h1;
  ...
  public float[][][] getFilterBank(int n1, int n2) {
    int nb = _nd-1;
    float[][][] fb = new float[nb][n2][n1];
    float pi = (float)Math.PI;
    float sigu = 1;
    float sigv = 8;
    float r1 = 0.5f*n1;
    ...
    for (int ib=0; ib<nb; ++ib) {
      ...
      for (int i1=0; i1<n1; ++i1) {
        float x1 = i1-r1;
        float x2 = i2-r2;
        float xu = x1*u1+x2*u2;
        float xv = x1*v1+x2*v2;
        xu = xu*xu*sigu;
        xv = xv*xv*sigv;
        fb[nb-ib-1][i2][i1] = exp(xu+xv);
      }}
    }
    return fb;
  }
