...
seist = np.fromfile(fileName+"seist.dat", dtype=np.single)
...
L6 = 128  # fully connected layer
L7 = 256  # fully connected layer
L8 = 256  # fully connected layer

W1 = tf.Variable(tf.truncated_normal([3, 3,  1, L1], stddev=0.1))  # 5x5 patch, 1 input channel, K output channels
...
W8 = tf.Variable(tf.truncated_normal([L7, 22], stddev=0.1))
B8 = tf.Variable(tf.ones([22])/22)
...
for i in range(101):
  ...
  ys = np.fromfile(fileName+"labelds"+str(i)+".dat", dtype=np.single)
  ...
  if(i%20==0):
    ...
    for i2 in range(102):
      print i2
      for i1 in range(101):
        pd = sess.run(predict, {X: [seista[k]], tst: True})
        fault[i2][i1] = max(pd[0])
        k = k+1
    fault.tofile(fileName+"fault"+str(i)+".dat", format="%4")
