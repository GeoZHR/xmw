...
tys = np.fromfile(fileName+"label1.dat", dtype=np.single)
txs = txs.reshape(100,32,32)
tys = tys.reshape(100,1)
...
W9 = tf.Variable(tf.truncated_normal([L8, 1], stddev=0.1))
B9 = tf.Variable(tf.constant(0.1, tf.float32, [1]))
...
YY = tf.reshape(Y7, shape=[-1, 8 * 8 * L7])
...
update_ema = tf.group(update_ema1, update_ema2, update_ema3, update_ema4, update_ema5, update_ema6, update_ema7, update_ema8)
...
for i in range(200):
  ...
  ys = np.fromfile(fileName+"label"+str(i)+".dat", dtype=np.single)
  ys = ys.reshape(100,1)
  ...
  if(i%20==0):
    ...
    p,a,c = sess.run([predict, accuracy, cross_entropy], {X: xsa, Y_: ys, tst: True, iter: i, pkeep: 1.0, pkeep_conv: 1.0})
    print p
    print ys
