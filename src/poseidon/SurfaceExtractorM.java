...
public class SurfaceExtractorM {
  ...
  public void surfaceUpdateFromSlopes
    (float[][][] ep, float[][][] p ,float[][][] q,float[][] sfr,
  ...
  private void surfaceCorrect(float lmt, float[][] surf) {
  ...
  private void surfaceCorrect(float lmt, float[][] surft, float[][] surf) {
    int n2 = surf.length;
    int n1 = surf[0].length;
    for(int i1=0; i1<n1; ++i1) {
      for (int i2=0; i2<n2; ++i2) {
        if (surf[i2][i1]<surft[i2][i1]) surf[i2][i1]=surft[i2][i1];
        if (surf[i2][i1]<0.f) surf[i2][i1]=0.f;
        if (surf[i2][i1]>lmt) surf[i2][i1]=lmt;
      }
    }
