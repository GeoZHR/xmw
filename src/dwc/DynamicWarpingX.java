...
public class DynamicWarpingX {
   ...
   */
  public DynamicWarpingX(int shiftMin, int shiftMax) {
   ...
   */
  public void setControlPoints(int n1, int[] cp, int[] cs) {
    _cp = cp;
    _cs = cs;
    _lmins = fillint(_lmin,n1);
    _lmaxs = fillint(_lmax,n1);
    float strain = 1f/(float)_bstrain1;
    int np = cp.length;
    ...
    for (int i1=0; i1<n1; ++i1) {
      System.out.println("lmin="+_lmins[i1]);
      System.out.println("lmax="+_lmaxs[i1]);
    }

  }

  public float[][] findSamples(float[][] d) {
    int n1 = d.length;
    int nl = d[0].length;
  ...
  public void smoothErrors1X(float[][][] e, float[][][] es) {
    smoothErrors1(_bstrain1,e,es);
    normalizeErrorsX(es);
    normalizeErrors(es);
  }
  private void normalizeErrorsX(float[][][] e) {
    int n2 = e.length;
  ...
  public void normalizeErrors(float[][] e) {
    ...
    float emax = FLT_MIN;
    for (int i1=0; i1<n1; ++i1) {
      int ilb = _lmins[i1]-_lmin;
      int ile = _lmaxs[i1]-_lmin;
  ...
  private int[] _cp, _cs; // known points and shifts
  private int _lmins[],_lmaxs[]; // min,max lags vary with indexes
  ...
  private void computeErrors(float[] f, float[] g, float[][] e) {
    ...
    for (int i1=0; i1<n1; ++i1) {
      int illb = _lmins[i1]-_lmin;
      int ille = _lmaxs[i1]-_lmin+1;
      int illo = max(illb,  -_lmin-i1); // see notes
      int ilhi = min(ille,n1-_lmin-i1); // above
      ...
      for (int il=illb; il<ille; ++il) {
        if (il<illo || il>=ilhi) {
          if (average) {
            ...
            int k1 = (il<illo)?-_lmins[i1]-il:n1m-_lmins[i1]-il;
            if (reflect)
              k1 += k1-i1;
            if (0<=k1 && k1<n1) {
              e[i1][il] = e[k1][il];
  ...
  private void accumulate(int dir, int b, float[][] e, float[][] d) {
    ...
    int ie = (dir>0)?ni:-1;
    int is = (dir>0)?1:-1;
    int ilb = _lmins[ib]-_lmin;
    int ile = _lmaxs[ib]-_lmin;
    for (int il=ilb; il<=ile; ++il)
      d[ib][il] = 0.0f;
    for (int ii=ib; ii!=ie; ii+=is) {
      int ji = max(0,min(nim1,ii-is));
      int jb = max(0,min(nim1,ii-is*b));
      int lbi = _lmins[ii]-_lmin;
      int lei = _lmaxs[ii]-_lmin;

      int lbj = _lmins[ji]-_lmin;
      int lej = _lmaxs[ji]-_lmin;

      int lbb = _lmins[jb]-_lmin;
      int leb = _lmaxs[jb]-_lmin;
      for (int il=lbi; il<=lei; ++il) {
        ...
        for (int kb=ji; kb!=jb; kb-=is) {
          int ilmk = il-1;
          int ilpk = il+1;
          int lbk = _lmins[kb]-_lmin;
          int lek = _lmaxs[kb]-_lmin;
    ...
    int dir, int b, int lmin, float[][] d, float[][] e, float[] u) 
  {
    ...
    int is = (dir>0)?1:-1;
    int ii = ib;
    int il = _lmins[ii]-_lmin;
    float dl = d[ii][il];
    for (int jl=_lmins[ii]-_lmin+1; jl<=_lmaxs[ii]-_lmin; ++jl) {
      if (d[ii][jl]<dl) {
        dl = d[ii][jl];
        il = jl;
      }
    ...
    while (ii!=ie) {
      ...
      int lbb = _lmins[jb]-_lmin;
      int leb = _lmaxs[jb]-_lmin;
      int lbi = _lmins[ji]-_lmin;
      int lei = _lmaxs[ji]-_lmin;
      if (il<lbi) il = lbi;
      if (il>lei) il = lei;
      int ilm1 = il-1; if (ilm1<lbb) ilm1 = lbb;
      int ilp1 = il+1; if (ilp1>leb) ilp1 = leb;
  ...
  public void setErrors(float em, float[][] e) {
    int n1 = e.length;
    int nl = e[0].length;
    for (int i1=0; i1<n1; ++i1) {
      int ll = _lmins[i1]-_lmin;
      int lh = _lmaxs[i1]-_lmin;
      for (int il=0; il<ll; ++il) 
        e[i1][il] = em;
      for (int il=lh+1; il<nl; ++il) 
        e[i1][il] = em;
    }
    int np = _cp.length;
    for (int ip=0; ip<np; ++ip) {
