...
def goHorizonS():
  ...
  if not plotOnly:
    ...
    wp = readImage(epfile)
    wp = pow(wp,4)
    c1 = rampfloat(50,2,ns)
    c2 = fillfloat(2962,ns)
    c3 = fillfloat(617,ns)
    hv = HorizonVolume()
    hv.setCG(0.01,100)
    hv.setExternalIterations(12)
  ...
  for ih in range(0,ns,5):
    title = "Slice "+str(ih)
    plot2(s2,s3,has[ih],cmin=-4,cmax=4,title=title,png=title)
    #plot3(gx,surf=hs[ih],cmin=-4,cmax=4.0,png="sf150")
  '''
  ...
  gx = copy(220,n2,n3,0,0,0,gx)
  c1 = Sampling(220)
  plot3p(s1,s2,s3,gx,hv=hs,k1=150,k2=500,k3=500,cmin=-2,cmax=2.0,png="hcs")
  '''
...
def plot3p(s1,s2,s3,f,g=None,hv=None,k1=None,k2=None,k3=None,cmap=ColorMap.GRAY,
  ...
  frame.setFontSize(18)#ForSlide(1.0,0.8)
