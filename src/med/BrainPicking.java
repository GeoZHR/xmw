...
/**
 * 3D brain picking in polar coordinates
 * @author Xinming Wu 
 * @version 2016.03.23
 */

public class BrainPicking {
 
  public BrainPicking(int gate, float an) {
    _gate = gate;
    _an = an;
  }


  public float[][][] applyForSkull(float[][][] v) {
    int sp = findMax(v[0][0]);
    float[][][] vt = copy(v);
    float[] pa = accumulateInline(sp,vt);
    vt = div(1f,vt);
    vt = sub(vt,min(vt));
    vt = div(vt,max(vt));
    accumulateCrossline(pa,vt);
    vt = div(1f,vt);
    vt = sub(vt,min(vt));
    vt = div(vt,max(vt));
    return applyForSurfaceInline(sp,2,2,vt);
  }

  public float[][][] applyForBrain(float[][] sk, float[][][] v) {
    int n3 = v.length;
    int n2 = v[0].length;
    int n1 = v[0][0].length;
    float[][][] vt = copy(v);
    ...
    for (int i2=0; i2<n2; ++i2) {
      int sk1 = round(sk[i3][i2])-2;
      sk1 = min(sk1,n1);
      for (int i1=sk1; i1<n1; ++i1)
        vt[i3][i2][i1] = 0f;
    }}
    int sp = findMax(vt[0][0]);
    float[] pa = accumulateInline(sp,vt);
    vt = div(1f,vt);
    vt = sub(vt,min(vt));
    vt = div(vt,max(vt));
  ...
  public float[][][] refineBrain(int d, float[][] br, float[][][] v) {
    int n3 = v.length;
    int n2 = v[0].length;
    int n1 = v[0][0].length;
    float[][][] vt = fillfloat(0f,4*d+1,n2,n3);
    for (int i3=0; i3<n3; ++i3) {
    for (int i2=0; i2<n2; ++i2) {
      int k1 = round(br[i3][i2]);
      for (int d1=-3*d;d1<=d;++d1) {
        int i1 = k1+d1;
        if(i1>=0 && i1<n1)
          vt[i3][i2][3*d+d1] = v[i3][i2][i1];
      }
    }}
    int sp = findMax(vt[0][0]);
    float[][][] sfs = applyForSurfaceInline(sp,2,2,vt);
    sfs = sub(sfs,3*d);
    sfs[0] = add(sfs[0],br);
    sfs[1] = add(sfs[1],br);
    return sfs;
  ...
  public float[] accumulateInline(final int i1p, final float[][][] vel) {
    final int n3 = vel.length;
    final int n2 = vel[0].length;
    final int n1 = vel[0][0].length;
    final float[] pf = new float[n3];
    final float[] pb = new float[n3];
    final float[] pa = new float[n3];
    ...
    public void compute(int i2) {
      ...
      if (i2==0) {
        forwardPick(i1p,w1,tf,pf);
        backwardPick(i1p,w1,tb,pb);
      } else {
        forwardAccumulate(i1p,w1,tf);
        backwardAccumulate(i1p,w1,tb);
      }
      for (int i3=0; i3<n3; ++i3) {
      for (int i1=0; i1<n1; ++i1) {
    ...
    for (int i3=0; i3<n3; ++i3) 
      pa[i3] = 0.5f*(pf[i3]+pb[i3]);
    return pa;
  }


  public void accumulateCrossline(float[] p, final float[][][] vel){
    final int n3 = vel.length;
    final int n2 = vel[0].length;
    final int n1 = vel[0][0].length;
    ...
    public void compute(int i3) {
      ...
      forwardAccumulate(i0,w2,tf);
      i0 = findMin(tf[n2-1]);
      i0 = min(i0,n1-1); i0 = max(i0,0);
      backwardAccumulate(i0,w2,tb);
      for (int i2=0; i2<n2; ++i2) {
      for (int i1=0; i1<n1; ++i1) {
  ...
  public void backwardAccumulate(int i0, float[][] wx, float[][] tx) {
    int n2 = wx.length;
    int n1 = wx[0].length;
    float[][] what = new float[n2][n1];
    float[] prev = new float[n1];
  ...
  public void backwardPick(int i0, float[][] wx, float[][] tx, float[] p) {
    int n2 = wx.length;
    int n1 = wx[0].length;
    float[][] what = new float[n2][n1];
    float[] prev = new float[n1];
  ...
  public void forwardAccumulate(int i0, float[][] wx, float[][] tx) {
    int n2 = wx.length;
    int n1 = wx[0].length;
    float[][] what = new float[n2][n1];
    float[] prev = new float[n1];
  ...
  public void forwardPick(int i0, float[][] wx, float[][] tx, float[] p) {
    int n2 = wx.length;
    int n1 = wx[0].length;
    float[][] what = new float[n2][n1];
    float[] prev = new float[n1];
  ...
  private int findMax(float[] x) {
    int ip = 0;
    float xm = x[0];
    int n1 = x.length;
    for (int i1=1;i1<n1;++i1) {
      float xi = x[i1];
      if(xi>xm) {ip = i1;xm = xi;}
    }
    return ip;
  }

  private int findMin(float[] x) {
    int ip = 0;
    float xm = x[0];
    int n1 = x.length;
    for (int i1=1;i1<n1;++i1) {
