...
public class FakeData {
  ...
  public static float[][][][] seismicAndSlopes3d2015A(
    ...
    // Faulting transforms
    float r1a = 0.0f*n1, r2a = 0.6f*n2, r3a = 0.5f*n3;
    float r1b = 0.0f*n1, r2b = 0.2f*n2, r3b = 0.3f*n3;
    float r1c = 0.3f*n1, r2c = 0.8f*n2, r3c = 0.5f*n3;
    float phia =  190.0f, thetaa = 75.0f; if (conjugate) phia += 180.0f;
    float phib =  10.0f, thetab = 75.0f;
    float phic = 190.0f, thetac = 75.0f;
    ...
    p[2] = copy(n1,n2,n3,p[2]);
    p[3] = copy(n1,n2,n3,p[3]);
    float[][][] hs = new float[][][]{hz,ha};
    return new float[][][][]{p[0],p[2],p[3],hs};
  }
   ...
   * @return transformed image q(x) = p(f(x) and normal vectors.
   */
  private static float[][][][] applyX(final T3 t, final float[][][][] p) {
    ...
    //for (int i3=0; i3<n3; ++i3) {
    final float alpha = (float)(Math.PI/18f);
    Parallel.loop(n3,new Parallel.LoopInt() {
    public void compute(int i3) {
      for (int i2=0; i2<n2; ++i2) {
        for (int i1=0; i1<n1; ++i1) {
          ...
          float c1i = t2i*sin(alpha)+t1i*cos(alpha);
          float c2i = t2i*cos(alpha)-t1i*sin(alpha);
          float c3i = t3i;
          float q1i = d.d11*c1i+d.d21*c2i+d.d31*c3i;
          float q2i = d.d12*c1i+d.d22*c2i+d.d32*c3i;
          float q3i = d.d13*c1i+d.d23*c2i+d.d33*c3i;
  ...
  private static class VerticalShear3X implements T3 {
    ...
    public C3 f(float x1, float x2, float x3) {
      ...
      C2 c2 = _s2.f(x2,x3);
      //x1 -= c1*c2.c1;
      float alpha = (float)(Math.PI/18f);
      x1 -= (c1*c2.c1+x2*sin(alpha));
      return new C3(x1,x2,x3);
    }
    public D3 df(float x1, float x2, float x3) {
      float c1 = _s1.f(x1);
  ...
  private static void addChannelsXXX(
    float[][][][] r, float[][] hz, float[][] ha) {
    int k1 = 70;
    int n3 = r[0].length;
    int n2 = r[0][0].length;
    for (int i3=0; i3<n3; ++i3) {
