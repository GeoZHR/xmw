 ...
 * @version 2016.06.15
 */
public class DynamicProgrammingS {
  ...
  public void setStrainMax(float[][] p) {
    _p = p;
  }

  public void setStrainMax(float[][][] p2, float[][][] p3) {
    _p2 = p2;
    _p3 = p3;
  }
  ...
  public void setControlPoints(
    int n1, int n2, int n3, int b, int[] k1, int[] k2, int[] k3) {
    _lbs = fillint(0,n2,n3);
    _les = fillint(n1-1,n2,n3);
    int np = k1.length;
    for (int ip=0; ip<np; ++ip) {
      int p1 = k1[ip];
      int p2 = k2[ip];
      ...
      for (int i3=0; i3<n3; ++i3) {
        lb = p1;le = p1;
        for (int i2=p2+1; i2<n2; ++i2) {
          lb -= b; le += b;
          if (lb>_lbs[i3][i2]) {_lbs[i3][i2]=lb;}
          if (le<_les[i3][i2]) {_les[i3][i2]=le;}
        }
        lb = p1; le = p1;
      ...
      for (int i2=0; i2<n2; ++i2) {
        lb = p1; le = p1;
        for (int i3=p3+1; i3<n3; ++i3) {
          lb -= b;le += b;
          if (lb>_lbs[i3][i2]) {_lbs[i3][i2]=lb;}
          if (le<_les[i3][i2]) {_les[i3][i2]=le;}
        }
        lb = p1; le = p1;
        for (int i3=p3-1; i3>=0; --i3) {
          lb -= b; le += b;
          if (lb>_lbs[i3][i2]) {_lbs[i3][i2]=lb;}
          if (le<_les[i3][i2]) {_les[i3][i2]=le;}
        }
  ...
  public void setControlPoints(
    int n1, int n2, int n3, float b, int[] k1, int[] k2, int[] k3, float[][][] e) {
    int np = k1.length;
    _lbs = fillint(0,n2,n3);
    _les = fillint(n1-1,n2,n3);
    for (int ip=0; ip<np; ++ip) {
      ...
      pi *= 2f;
      for (float phi=0f; phi<pi; phi+=0.001f){
        float lb = p1;
        float le = p1;
        float s2 = sin(phi);
        float s3 = cos(phi);
        for (int d=1; d<n2; ++d) {
          lb -= b; le += b;
          int i2 = round(p2+s2*d);
          int i3 = round(p3+s3*d);
          i2 = max(i2,0);
          i3 = max(i3,0);
          i2 = min(i2,n2-1);
          i3 = min(i3,n3-1);
          int lbk = round(lb);
          int lek = round(le);
          if(lbk>_lbs[i3][i2]) _lbs[i3][i2] = lbk;
          if(lek<_les[i3][i2]) _les[i3][i2] = lek;
          for (int i1=0; i1<lbk; i1++) {
            e[i3][i2][i1] = emax;
          }
          for (int i1=n1-1; i1>lek; --i1) {
            e[i3][i2][i1] = emax;
          }
      }
    }
  ...
  public void findPath(float[][] e, float[] u) {
    ...
    for (int is=0; is<_esmooth; ++is)
      smoothErrors1(_lmins,_lmaxs,_p,e,e);
    float[][] d = accumulateForward(e);
    backtrackReverse(_lmins,_lmaxs,_p,d,e,u);
    smoothShifts(u,u);
  }
  ...
  public void findSurface(float[][][] e, float[][] u) {
    ...
    final float[][] uf = u;
    if(_lbs==null) _lbs = fillint(0,n1,n2);
    if(_les==null) _les = fillint(nl-1,n1,n2);
    for (int is=0; is<_esmooth; ++is)
      smoothErrors(e,e);
    ...
    public void compute(int i2) {
      ...
      accumulateForward(_lbs[i2],_les[i2],_p3[i2],e[i2],d);
      backtrackReverse(_lbs[i2],_les[i2],_p3[i2],d,e[i2],uf[i2]);
    }});
    smoothShifts(u,u);
  }
  ...
  public float[][] smoothErrors(float[][] e) {
    float[][] es = like(e);
    smoothErrors1(_lmins,_lmaxs,_p,e,es);
    return es;
  }
  ...
  public float[][] accumulateForward(float[][] e) {
    float[][] d = like(e);
    accumulateForward(_lmins,_lmaxs,_p,e,d);
    return d;
  }
   ...
   */
  public void accumulateForward(int[] lbs, int[] les, float[][] p, 
    float[][] e, float[][] d) {
    accumulate( 1,lbs,les,p,e,d);
  }
  ...
  public void accumulateForward1(float[][][] e, float[][][] d) {
    int n2 = e.length;
    for (int i2=0; i2<n2; ++i2)
      accumulateForward(_lbs[i2],_les[i2],_p3[i2],e[i2],d[i2]);
  }
   ...
   */
  public void backtrackReverse(int[] lbs, int[] les, 
    float[][] p, float[][] d, float[][] e, float[] u) 
  {
    backtrack(-1,lbs,les,p,d,e,u);
  }
  ...
  public void backtrackReverse1(float[][][] d, float[][][] e, float[][] u) {
    int n2 = d.length;
    for (int i2=0; i2<n2; ++i2)
      backtrackReverse(_lbs[i2],_les[i2],_p3[i2],d[i2],e[i2],u[i2]);
  }
  ...
  public void backtrackReverse2(float[][][] d, float[][][] e, float[][] u) {
    ...
    for (int i1=0; i1<n1; ++i1) {
      ...
      backtrack(-1,lb1,le1,p21,di1,ei1,ui1);
      for (int i2=0; i2<n2; ++i2)
        u[i2][i1] = ui1[i2];
    }
  }
  ...
  private int[][] _lbs = null;
  private int[][] _les = null;
  ...
  private void accumulate(
    int dir, int[] lbs, int[] les, float[][] strain, float[][] e, float[][] d) 
  {
    int ni = e.length;
    int nl = e[0].length;
    int nim1 = ni-1;
    ...
    for (int ii=ib; ii!=ie; ii+=is) {
      ...
      for (int il=0; il<nl; ++il) {
        float si = strain[ii][il]+0.2f;
        si = min(si,1.0f);
        si = max(si,0.1f);
        int b = (int)ceil(1.0/si);
        int jb = max(0,min(nim1,ii-is*b));
        //int lbb = lbs[jb];
        //int leb = les[jb];
        //int jl = il;
        ...
        int ilp1 = il+1; if (ilp1>nl-1) ilp1 = nl-1;
  ...
  private void backtrack(
    int dir, int[] lbs, int[] les, float[][] strain, float[][] d, float[][] e, float[] u) 
  {
    //float ob = 1.0f/b;
    int nl = d[0].length;
    int ni = d.length;
    ...
    while (ii!=ie) {
      ...
      int jb = max(0,min(nim1,ii+is*b));
      int ilm1 = il-1; if (ilm1<0) ilm1 = 0;
      int ilp1 = il+1; if (ilp1>=nl) ilp1 = nl-1;
      float dm = d[jb][ilm1];
      float di = d[ji][il  ];
      float dp = d[jb][ilp1];
      for (int kb=ji; kb!=jb; kb+=is) {
  ...
  private void smoothErrors1(
    int[] lbs, int[] les, float[][] strain, float[][] e, float[][] es) 
  {
    int nl = e[0].length;
    int n1 = e.length;
    float[][] ef = new float[n1][nl];
    float[][] er = new float[n1][nl];
    accumulate( 1,lbs,les,strain,e,ef);
    accumulate(-1,lbs,les,strain,e,er);
    for (int i1=0; i1<n1; ++i1)
      for (int il=0; il<nl; ++il)
   ...
   * @param es output array of smoothed alignment errors.
   */
  private void smoothErrors1(float[][][] e, float[][][] es) {
    final int n2 = e.length;
    final float[][][] ef = e;
    final float[][][] esf = es;
    final float[][][] p3f = _p3;
    ...
    Parallel.loop(n2,new Parallel.LoopInt() {
    public void compute(int i2) {
      smoothErrors1(lbf[i2],lef[i2],p3f[i2],ef[i2],esf[i2]);
    }});
  }
   ...
   * @param es output array of smoothed alignment errors.
   */
  private void smoothErrors2(float[][][] e, float[][][] es) {
    final int nl = e[0][0].length;
    final int n1 = e[0].length;
    final int n2 = e.length;
    final float[][][]  ef = e;
    ...
    public void compute(int i1) {
      ...
      if (ls==null) lsu.set(ls=new int[2][n2]);
      float[][]  e1 = ee[0];
      float[][] es1 = ee[1];
      float[][] ef1 = ee[2];
      float[][] er1 = ee[3];
      ...
      accumulate( 1,lbs,les,st1,e1,ef1);
      accumulate(-1,lbs,les,st1,e1,er1);
      for (int i2=0; i2<n2; ++i2) {
        for (int il=0; il<nl; ++il) {
