...
public class ImplicitSurface {
    ...
    float[][] st, float[][] u1, float[][] u2)
  {
    int n2 = u1.length;
    int n1 = u1[0].length;
    float[][] w = new float[n2][n1]; // right-hand side
  ...
  public void setPoints(
    float[][] st, float[][] u1, float[][] u2, float[][] sw, float[][] sp){
    int n2 = st.length;
    int n1 = st[0].length;
    float d = 3f;
    ...
    for (int i1=0; i1<n1; ++i1) {
      float sti = st[i2][i1];
      if(sti<=0.0f){continue;}
      float u1i = u1[i2][i1]*d;
      float u2i = u2[i2][i1]*d;
      int i1p = round(i1+u1i);
      int i2p = round(i2+u2i);
      int i1m = round(i1-u1i);
      int i2m = round(i2-u2i);
      i1p = min(i1p,n1-1);
      i2p = min(i2p,n2-1);
      i1m = min(i1m,n1-1);
      i2m = min(i2m,n2-1);
