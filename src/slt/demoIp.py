...
def main(args):
  ...
  #goCorrelation()
def goCorrelation():
  fx = readImage2d(gxfile)
  ss = SaltScanner()
  pr = ss.applyForCoorelation(10,fx)
  print min(pr)
  print max(pr)
  pr = sub(pr,min(pr))
  pr = div(pr,max(pr))
  ...
  fx = sub(fx,min(fx))
  fx = div(fx,max(fx))
  plot2(pr,s1,s2,cmin=0.0001,cmax=1.0)
  plot2(fx,s1,s2,cmin=0.0001,cmax=0.6)
...
def goSaltSurfer():
  ...
  mk = pow(st,7) 
  ip = ImplicitSurface()
  ip.setSmoothings(20,20,20)
  sf = ip.saltIndicator(mk,g1,g2)
  print min(sf)
  print max(sf)
  c1 = Sampling(n1)
  c2 = Sampling(n2)
  plot2(gx,c1,c2,st,cmin=0.1,cmax=1,cmap=jetRamp(1.0),
        label="Thinned salt likelihood")
  plot2(gx,c1,c2,sf,u=sf,cmin=-18,cmax=18,cmap=bwrRamp(1.0),
