...
public class Flattener3R {
   ...
   */
  public Mappings getMappingsFromRgt(
    Sampling s1, Sampling s2, Sampling s3, final float[][][] u1)
  {
    // Sampling parameters.
    final int n1 = s1.getCount();
    final int n2 = s2.getCount();
  ...
  public void computeRgt(
    float[][][] wp, float[][][] u1, float[][][] u2, float[][][] u3, 
    float[][][] cs,   int[][][] uc, float[][][] r1) 
  {
    int n3 = u2.length;
    int n2 = u2[0].length;
    int n1 = u2[0][0].length;
    for (int i3=0; i3<n3; ++i3) {
    for (int i2=0; i2<n2; ++i2) {
    for (int i1=0; i1<n1; ++i1) {
      r1[i3][i2][i1] = i1;
    ...
    VecArrayFloat3 vr = new VecArrayFloat3(r1);
  ...
  private float[][][] setWeightsFromUnconformities(float[][][] wp, int[][][] uc) {
    ...
    if (uc==null) {
      ...
      int nc = uc[0].length;
      float[][][] w1 = fillfloat(0.01f,n1,n2,n3);
  ...
  private static class M3 implements CgSolver.A {
    ...
    private float[][][] _wp;
    private float[][] _k1 = null;
    private float[][] _k2 = null;
    private float[][] _k3 = null;
  }
  ...
  */
  ...
  private static void cleanRgt(float[][][] u1) {
    int n3 = u1.length;
    int n2 = u1[0].length;
    ...
    for (int i3=0; i3<n3; ++i3) {
      for (int i2=0; i2<n2; ++i2) {
        for (int i1=1; i1<n1; ++i1) {
          if (u1[i3][i2][i1]<=u1[i3][i2][i1-1]+0.01f)
            u1[i3][i2][i1] = u1[i3][i2][i1-1]+0.01f;
        }
      }
    }
  }
