 ...
 * Weighted by unconformity likelihoods to estimate discontious 
 * normal vectors at unconformities.
 * 
...
public class WeightedLocalOrientFilter {
   ...
   */
  public WeightedLocalOrientFilter(double sigma1, double sigma2) {
   ...
   */
  public WeightedLocalOrientFilter(double sigma1, double sigma2, double sigma3) {
    _gpSigma1 = (float)sigma1;
    _gpSigma2 = (float)sigma2;
    _gpSigma3 = (float)sigma3;
    setGradientSmoothing(1.0);
  }

  public void setScaleFactor(float[][] sg, float[][] sgp) {
    _sg2  = sg;
    _sgp2 = sgp;
  }
  public void setScaleFactor(float[][][] sg, float[][][] sgp) {
  ...
  {
    _gSigma1 = (float)sigma1;
    _gSigma2 = (float)sigma2;
    _gSigma3 = (float)sigma3;
  }
    ...
    float[][] el)
  {
    ...
    float[][] xt = copy(x);
    smooth1(_gSigma2,_sg2,xt);
    smooth2(_gSigma2,_sg2,xt);
    //RecursiveGaussianFilter rgf = new RecursiveGaussianFilter(1.0);
    //rgf.apply10(x,g1);
    //rgf.apply01(x,g2);
    Gradient.center2D(xt,g1,g2);
    // Gradient products.
    float[][] g11 = g1;
    float[][] g22 = g2;
    ...
    for (float[][] g:gs) {
      smooth1(_gpSigma1,_sgp2,g);
      smooth2(_gpSigma2,_sgp2,g);
    }
    ...
    float[][][] ep, float[][][] el)
  {
    ...
    float[][][] xt = copy(x);
    smooth1(_gSigma1,_sg3,xt);
    smooth2(_gSigma1,_sg3,xt);
    smooth3(_gSigma1,_sg3,xt);
    Gradient.center3D(xt,g1,g2,g3);
    ...
    for (float[][][] g:gs) {
      smooth1(_gpSigma1,_sgp3,g);
      smooth2(_gpSigma2,_sgp3,g);
      smooth3(_gpSigma3,_sgp3,g);
    }
  ...
  ///////////////////////////////////////////////////////////////////////////
  // private
  private float _gSigma1;
  private float _gSigma2;
  private float _gSigma3;
  private float _gpSigma1;
  private float _gpSigma2;
  private float _gpSigma3;
  private float[][]   _sg2;
  private float[][]   _sgp2;
  private float[][][] _sg3;
  private float[][][] _sgp3;
  ...
  private static void smooth1(float sigma, float[][] s, float[][] x) {
    ...
    int n2 = x.length;
    int n1 = x[0].length;
    float c = sigma*(sigma+1.0f)/6.0f;
    float[] yt = zerofloat(n1); 
    LocalSmoothingFilter lsf = new LocalSmoothingFilter();
    for (int i2=0; i2<n2; ++i2) {
      lsf.apply(c,s[i2],x[i2],yt);
      copy(yt,x[i2]);
    }
  }
  ...
  // for test only
  private void plot2D(float[][] x, String title) {
