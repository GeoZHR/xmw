...
public class LocalSlopeFinder {
  ...
  public void padBounds(int nb, float[][][] f) {
    int n3 = f.length;
    int n2 = f[0].length;
    int n1 = f[0][0].length;
    ...
    for (int i1=0; i1<n1; ++i1) {
      for (int i2=0; i2<nb; ++i2) {
        f[i3][i2][i1] = f[i3][nb][i1];
      }
      for (int i2=n2-nb; i2<n2; ++i2) {
        f[i3][i2][i1] = f[i3][n2-nb-1][i1];
      }
    }}
    for (int i2=0; i2<n2; ++i2) {
    for (int i1=0; i1<n1; ++i1) {
      for (int i3=0; i3<nb; ++i3) {
        f[i3][i2][i1] = f[nb][i2][i1];
      }
      for (int i3=n3-nb; i3<n3; ++i3) {
        f[i3][i2][i1] = f[n3-nb-1][i2][i1];
      }
    }}
  }
