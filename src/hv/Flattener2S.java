...
public class Flattener2S {
    ...
    Sampling s1, Sampling s2, float[][] p2, float[][] el, float[][][] c) 
  {
    ...
    A2 a2 = new A2(smoother2,_weight1,el,p2,c);
  ...
  private static class A2 implements CgSolver.A {
    A2(Smoother2 s2, float w1, float[][] wp, float[][] p2, float[][][] c) {
      _s2 = s2;
      _w1 = w1;
      _wp = wp;
      _p2 = p2;
    ...
    public void apply(Vec vx, Vec vy) {
      ...
      _s2.apply(z);
      add(-_sc,z,y);
      applyLhs(_w1,_wp,_p2,_c,z,y);
      _s2.applyTranspose(y);
      add(_sc,x,y);
    }
    private Smoother2 _s2;
    private float _w1,_sc;
    private float[][] _wp;
    private float[][] _p2;
    private float[][][] _c;
  }
  ...
  private static void smooth1(float sigma, float[][] s, float[][] x) {
    ...
    for (int i2=0; i2<n2; ++i2) {
      if (s!=null) {
        lsf.apply(c,s[i2],x[i2],x[i2]);
      } else {
        lsf.apply(c,x[i2],x[i2]);
      }
    }
  }
  ...
  private static void makeRhs(float[][] wp, float[][] p2, 
    float[][][] c, float[][] y) {
    int n1 = y[0].length;
    int n2 = y.length;
    ...
    if (c!=null) constrainRhs(c,y);
  }
  private static void applyLhs(
    float w1, float[][] wp, float[][] p2, float[][][] c, 
    float[][] x, float[][] y) 
  {
    int n1 = x[0].length;
    int n2 = x.length;
    ...
    if (c!=null) constrainLhs(c,x,y);
  }

  private static void constrainLhs(float[][][] c, float[][] x, float[][] y) {
    float beta = 8f;
    float[][] c1 = c[0];
    float[][] c2 = c[1];
    int nc = c1.length;
    for (int ic=0; ic<nc; ++ic) {
      int np = c1[ic].length;
      int r1 = round(c1[ic][0]);
      int r2 = round(c2[ic][0]);
      for (int ip=1; ip<np; ++ip) {
        int k1 = round(c1[ic][ip]);
        int k2 = round(c2[ic][ip]);
        y[r2][r1] += beta*(x[r2][r1]-x[k2][k1]);
        y[k2][k1] += beta*(x[k2][k1]-x[r2][r1]);
      }
    }
  }

  private static void constrainRhs(float[][][] c, float[][] y) {
    float beta = 8f;
    float[][] c1 = c[0];
    float[][] c2 = c[1];
    int nc = c1.length;
  ...
  private void rearrange(float[][][] ks) {
    int ns = ks[0].length;
    System.out.println("ns="+ns);
    float[][][] kc = new float[2][ns][];
    for (int is=0; is<ns; ++is) {
      int np = ks[0][is].length;
      System.out.println("np="+np);
      kc[0][is] = new float[np];
      kc[1][is] = new float[np];
      rearrange(ks[0][is],ks[1][is]);
    }
  }

  private void rearrange(float[] k1, float[] k2) {
    int ai = 0;
    int np = k1.length;
