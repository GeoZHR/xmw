...
def main(args):
  ...
  #shiftsToRgt()
  #rgtToHv()
  #horizonVolume(88,415,8,130,30,"hv")
  fx = readImage("gm")
  fw = readImage("gmfc")
  fu = readImage("gmRgt")
  k1,k2,k3=88,406,5
  azimuth,elevation=130,30
  fx = gain(fx)
  fw = gain(fw)
  view3D(k1,k2,k3,azimuth,elevation,f=fx,png="seismic")
  view3D(k1,k2,k3,azimuth,elevation,g=fu,png="rgt")
  k1,k2,k3=71,406,5
  view3D(k1,k2,k3,azimuth,elevation,f=fx,png="seismic1")
  view3D(k1,k2,k3,azimuth,elevation,f=fw,png="flat")
  #seiVolume(k1,k2,k3,130,30,"seiSlides")
  #k1,k2,k3=88,406,5
  #fltVolume(k1,k2,k3,130,30,"fltSlides")
  #rgtVolume(k1,k2,k3,130,30,"rgtSlides")
  '''
  slices()
...
def flatten():
  ...
  kk = [k11,k12,k13]
  sc = SetupConstraints()
  sc.setForExtension(sigma1,sigma2,0.0)
  kk1 = sc.extend(k11,k12,k13,500,500,p2,p3,wh,f)
  kk2 = sc.extend(k21,k22,k23,500,500,p2,p3,wh,f)
  kk3 = sc.extend(k31,k32,k33,500,500,p2,p3,wh,f)
  kk4 = sc.extend(k41,k42,k43,500,500,p2,p3,wh,f)
  '''
  np1 = 140485
  np2 = 99547
  np3 = 140700
  ...
  k53 = [  0,  0,  0,  0,  0,  0,  0,  0]
  kk5 = sc.extend(k51,k52,k53,15,25,p2,p3,wh,f)
...
def shiftsToRgt():
  ...
  for i3 in range(n3):
    for i2 in range(n2):
      for i1 in range(n1):
        x1i = f1+i1*d1
        u1[i3][i2][i1] = x1i+sf[i3][i2][i1]*d1
  writeImage("gmRgt",u1)
  print "u1Min=",min(u1)
  print "u1Max=",max(u1)
def rgtToHv():
  u1 = readImage("gmRgt")
  x1 = zerofloat(n1,n2,n3)
  ii = InverseInterpolator(st,st)
  for i3 in range(n3):
    for i2 in range(n2):
...
def slices():
  ...
  k1 = h2
  plot3f(k1,k2,k3,fu,cbar,label1,gmap=jet,cint=0.2,gmin=rmin,gmax=rmax,png="hv")
  label1,cbar = "RGT","Time (s)"
  plot3f(k1,k2,k3,fx,cbar,label1,gmap=jet,cint=0.1,gmin=xmin,gmax=xmax,png="hv")
...
def rgbFromHeight(h,r,g,b):
  ...
  for i1 in range(n1):
    for i2 in range(n2):
      ...
      b[i2][i1] = htRGB[i+2] 
      i = i+3
def view3D(k1,k2,k3,azimuth,elevation,f=None,g=None,png=None):
  world = World()
  if f:
    ipg = addImageToWorld(s1,s2,s3,world,f,cmap=gray,cmin=-2.5,cmax=2.5)
  if g:
    ipg = addImageToWorld(s1,s2,s3,world,g,cmap=jet,cmin=1.45,cmax=2.3)
  ipg.setSlices(k1,k2,k3)
  makeFrame(s1,s2,s3,azimuth,elevation,world,png=png)

def seiVolume(k1,k2,k3,azimuth,elevation,png):
  f = readImage("gm")
  f = gain(f)
  world = World()
  print min(f)
...
def rgtVolume(k1,k2,k3,azimuth,elevation,png):
  f = readImage("gmRgt")
  world = World()
  print min(f)
  print max(f)
  ipg = addImageToWorld(s1,s2,s3,world,f,cmap=jet,cmin=1.45,cmax=2.3)
  ipg.setSlices(k1,k2,k3)
  makeFrame(s1,s2,s3,azimuth,elevation,world,png=png)

def horizonVolume(k1,k2,k3,azimuth,elevation,png):
  f = readImage("gmHv")
  world = World()
  ipg = addImageToWorld(s1,s2,s3,world,f,cmap=jet,cmin=1.6,cmax=1.89)
  ipg.setSlices(k1,k2,k3)
  makeFrame(s1,s2,s3,azimuth,elevation,world,png=png)
