...
def flattenWithSlopesAndCorrelations():
  ...
  gcf.setStrainMax(0.2)
  #pc = gcf.getTraceIndexes(5,150,n2) 
  fs = zerofloat(n1,n2)
  rgf = RecursiveExponentialFilter(1)
  rgf.apply1(f,fs)
  pc = gcf.getTraceIndexes(5,50,n2) 
  tc = gcf.findCorrelations(pc,fs)
  fl = Flattener2Dw()
  fl.setWeight1(0.0)
  fl.setIterations(0.01,500)
  fl.setSmoothings(8.0,8.0)
  #ks,dt=None,None
  fm = fl.getMappingsFromSlopesAndCorrelations(s1,s2,0.001,p2,wp,pc,tc)
  g = fm.flatten(f)
  plot(s1,s2,f,title="Input",png="f")
  plot(s1,s2,g,title="Flattened",png="g")
  plot(s1,s2,f,u=fm.u1,title="Input",png="f")
