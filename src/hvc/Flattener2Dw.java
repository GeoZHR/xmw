...
/**
 * Flattens locally linear features in a 2D image.
 * <p>
...
public class Flattener2Dw {
  ...
  public float[][][] findCorrelations(float[][] gx) {
    int n2 = gx.length;
    int n1 = gx[0].length;
    float[][][] us = new float[n2][n2-1][n1];
    return us;
  }
    ...
    Sampling s1, Sampling s2, float[][] p2, float[][] el) 
  {
    ...
    A2 a2 = new A2(smoother2,_weight1,0,el,p2,null);
    CgSolver cs = new CgSolver(_small,_niter);
    makeRhs(0,el,p2,null,null,b);
   ...
   * @param el array of linearities of image features.
   */
  public Mappings getMappingsFromSlopesAndCorrelations(
    Sampling s1, Sampling s2, float scale, float[][] p2, float[][] el, 
    int[][] ks, float[][] dt) 
  {
    // Sampling parameters.
    int n1 = s1.getCount();
    ...
    VecArrayFloat2 vr = new VecArrayFloat2(r);
    Smoother2 smoother2 = new Smoother2(n1,n2,_sigma1,_sigma2,el);
    A2 a2 = new A2(smoother2,_weight1,scale,el,p2,ks);
    CgSolver cs = new CgSolver(_small,_niter);
    makeRhs(scale,el,p2,ks,dt,b);
    smoother2.applyTranspose(b);
    cs.solve(a2,vb,vr);
    smoother2.apply(r);
    cleanShifts(r);
  ...
  private static class A2 implements CgSolver.A {
    A2(Smoother2 s2, float w1, float sc, float[][] wp, 
       float[][] p2, int[][] ks) {
      _s2 = s2;
      _w1 = w1;
      _sc = sc;
    ...
    public void apply(Vec vx, Vec vy) {
      ...
      if(_ks!=null) {screenLhs(_sc,_ks,z,y);}
  ...
  private static void makeRhs(
    float scale,
    float[][] wp, float[][] p2, int[][] ks, float[][] dt, float[][] y) {
    int n1 = y[0].length;
    int n2 = y.length;
    ...
    if(ks!=null) screenRhs(scale,ks,dt,y);
  }

  private static void screenRhs(float scale, 
    int[][] ks, float[][] dt, float[][] y) {
    int ns = ks.length;
    int n1 = y[0].length;
  ...
  private static void screenLhs(
    float scale, int[][] ks, float[][] x, float[][] y) {
    int ns = ks.length;
    int n1 = x[0].length;
    for (int is=0; is<ns; ++is) {
      int p2 = ks[is][0];
