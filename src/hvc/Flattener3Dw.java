...
/**
 * Flattens locally planar features in a 3D image.
 * <p>
...
public class Flattener3Dw {
    ...
    float[][][] p2, float[][][] p3, float[][][] ep) 
  {
    ...
    A3 a3 = new A3(smoother3,_weight1,0,ep,p2,p3,null);
    CgSolver cs = new CgSolver(_small,_niter);
    makeRhs(0,ep,p2,p3,null,null,b);
  ...
  public Mappings getMappingsFromSlopesAndCorrelations(
    Sampling s1, Sampling s2, Sampling s3, float scale,
    float[][][] p2, float[][][] p3, float[][][] ep, int[][] ks, float[][] ts) 
  {
    // Sampling parameters.
    final int n1 = s1.getCount();
    final int n2 = s2.getCount();
    ...
    VecArrayFloat3 vr = new VecArrayFloat3(r);
    Smoother3 smoother3 = new Smoother3(n1,n2,n3,_sigma1,_sigma2,_sigma3,ep);
    A3 a3 = new A3(smoother3,_weight1,scale,ep,p2,p3,ks);
    CgSolver cs = new CgSolver(_small,_niter);
    makeRhs(scale,ep,p2,p3,ks,ts,b);
    smoother3.applyTranspose(b);
    cs.solve(a3,vb,vr);
    smoother3.apply(r);
    cleanShifts(r);
  ...
  private static class A3 implements CgSolver.A {
    A3(Smoother3 s3, float w1, float sc, float[][][] wp, 
       float[][][] p2, float[][][] p3, int[][] ks) 
    {
      _s3 = s3;
      _w1 = w1;
      _sc = sc;
    ...
    int[][] ks, float[][] ts, float[][][] y) 
  {
    ...
    if(ks!=null) {screenRhs(scale,ks,ts,y);}
  }
  ...
  private static void screenLhs(
    final float scale, final int[][] ks, 
