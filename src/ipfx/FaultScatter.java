...
/**
 * Extends fault likelihoods, strikes, and dips from fault cells 
 * to recompute fault images.
 * Fault likelihoods are in the range [0,1], where 0 and 1
 ...
 * @version 2015.10.30
 */
public class FaultScatter {

  /**
   * Constructs a fault scatter with specified parameters.
   ...
   */
  public FaultScatter(double sigmaPhi, double sigmaTheta) {
    _sigmaPhi = sigmaPhi;
  ...
  public float[][][] scatter(
    ...
    */
    final FaultCell[][][] fca = sortCells(cells);
    for (int phi=0; phi<=360; ++phi) {
      System.out.println("phi="+phi);
      for (int it=0; it<91; ++it) {
        float[][][] flc = new float[n3][n2][n1];
        if(fca[phi][it].length<1){continue;}
        setFaultImage(flc,fca[phi][it]);
        Rotator r = new Rotator(phi,n1,n2,n3);
        float[][][] rfl = r.rotate(flc);
        smooth2(rfl);
        float[][][] flt = scatterTheta(it,rfl);
        final float[][][] flr = r.unrotate(flt);
        loop(n3,new LoopInt() {
        public void compute(int i3) {
          for (int i2=0; i2<n2; ++i2) {
            float[] f32  = fls[i3][i2];
            float[] fp32 = flr[i3][i2];
            for (int i1=0; i1<n1; ++i1) {
              float fpi = fp32[i1];
              f32[i1] += fpi;
            }
  ...
  private float[][][] scatterTheta(final float ti, final float[][][] flc) 
  {
    final int n1 = n1(flc); 
    final int n2 = n2(flc);
    final float[][][] flt = like(flc);
    final SincInterpolator si = new SincInterpolator();
    si.setExtrapolation(SincInterpolator.Extrapolation.CONSTANT);
    loop(n2,new LoopInt() {
    public void compute(int i2) {
      float[][] fl2 = extractSlice2(i2,flc);
      if (fl2==null) {return;}
      int n3 = fl2.length;
      float theta = toRadians(ti);
      float shear = -1.0f/tan(theta);
      float[][] fls = shear(si,shear,fl2);
      float sigma = (float)_sigmaTheta*sin(theta);
      RecursiveExponentialFilter ref = makeRef(sigma);
      ref.apply1(fls,fls);
      float[][] s2 = unshear(si,shear,fls);
      for (int i3=0,j3=i3lo(i2,flt); i3<n3; ++i3,++j3) {
        float[] s32 = s2[i3];
        float[] f32 = flt[j3][i2];
        for (int i1=0; i1<n1; ++i1) {
          f32[i1] = s32[i1];
  ...
  private void setFaultImage(float[][][] flc, FaultCell[] fca) {
    for (FaultCell fc:fca) {
      int i1 = fc.i1;
      int i2 = fc.i2;
      int i3 = fc.i3;
      flc[i3][i2][i1] = fc.fl;
  ...
  public FaultCell[][][] sortCells(FaultCell[] cells) {
    int[][] cpt = new int[361][91];
    for (FaultCell cell:cells) {
      int pi = round(cell.fp);
      int ti = round(cell.ft);
      cpt[pi][ti] += 1;
    }
    FaultCell[][][] fca = new FaultCell[361][91][];
    for (int p=0; p<=360; ++p) {
    for (int t=0; t<= 90; ++t) {
      int nc = cpt[p][t];
      fca[p][t] = new FaultCell[nc];
    }}
    int[][] ipt = new int[361][91];
    for (FaultCell cell:cells) {
      int p = round(cell.fp);
      int t = round(cell.ft);
      int k = ipt[p][t];
  ...
  // Horizontal smoothing of rotated snum,sden along axis 2.
  private void smooth2(final float[][][] fl) {
    final int n3 = n3(fl);
    final RecursiveExponentialFilter ref = makeRef(_sigmaPhi);
    loop(n3,new LoopInt() {
    public void compute(int i3) {
      float[][] s3 = extractSlice3(i3,fl);
      if (s3!=null) {
        ref.apply2(s3,s3); 
        restoreSlice3(i3,fl,s3);
      }
    }});
  }
