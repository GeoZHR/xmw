...
public class FaultScannerX {
   ...
   */
  public FaultScannerX(double sigmaPhi, double sigmaTheta) {
    _sigmaPhi = sigmaPhi;
  ...
  private float[][][][] scan(
    ...
    final int n3 = sem.length;
    final int n2 = sem[0].length;
    final int n1 = sem[0][0].length;
    final float[][][] fl = new float[n3][n2][n1];
    final float[][][] fp = new float[n3][n2][n1];
    final float[][][] ft = new float[n3][n2][n1];
    final float[][][] fc = new float[n3][n2][n1];
    ...
    for (int ip=0; ip<np; ++ip) {
      ...
      float[][][] rsem = r.rotate(sem);
      float[][][][] rgs = r.rotate(gs);
      smooth2(rsem);
      scanTheta(phi,thetaSampling,rsem,rgs);
      gs = r.unrotate(rgs);
    }
    final float[][][] g11 = gs[0];
    final float[][][] g12 = gs[1];
    final float[][][] g13 = gs[2];
    final float[][][] g22 = gs[3];
    final float[][][] g23 = gs[4];
    final float[][][] g33 = gs[5];
    loop(n3,new LoopInt() {
    public void compute(int i3) {
  ...
  private void smooth2(final float[][][] sem) {
    final int n3 = n3(sem);
    final RecursiveExponentialFilter ref = makeRef(_sigmaPhi);
    loop(n3,new LoopInt() {
    public void compute(int i3) {
      float[][] s3 = extractSlice3(i3,sem);
      if (s3!=null) {
        ref.apply2(s3,s3); 
        restoreSlice3(i3,sem,s3);
      }
    }});
  }
  ...
  private void scanTheta(
    float phi, Sampling thetaSampling, 
    final float[][][] sem, final float[][][][] gs) {
    final int n1 = n1(sem), n2 = n2(sem);
    final Sampling st = thetaSampling;
    final float[][] ts = computeTensors(phi,st);
    final SincInterpolator si = new SincInterpolator();
    si.setExtrapolation(SincInterpolator.Extrapolation.CONSTANT);
    loop(n2,new LoopInt() {
    public void compute(int i2) {
      float[][] sem2 = extractSlice2(i2,sem);
      float[][] g112 = extractSlice2(i2,gs[0]);
      float[][] g122 = extractSlice2(i2,gs[1]);
      float[][] g132 = extractSlice2(i2,gs[2]);
      float[][] g222 = extractSlice2(i2,gs[3]);
      float[][] g232 = extractSlice2(i2,gs[4]);
      float[][] g332 = extractSlice2(i2,gs[5]);
      if (sem2==null)
        return;
      int n3 = sem2.length;
      int nt = st.getCount();
      for (int it=0; it<nt; ++it) {
        ...
        float[][] sems = shear(si,shear,sem2);
        float sigma = (float)_sigmaTheta*sin(theta);
        RecursiveExponentialFilter ref = makeRef(sigma);
        ref.apply1(sems,sems);
        float[][] s2 = unshear(si,shear,sems);
        for (int i3=0; i3<n3; ++i3) {
        for (int i1=0; i1<n1; ++i1) {
          float scale = s2[i3][i1];
          g112[i3][i1] += scale*g11;
          g122[i3][i1] += scale*g12;
          g132[i3][i1] += scale*g13;
          g222[i3][i1] += scale*g22;
          g232[i3][i1] += scale*g23;
          g332[i3][i1] += scale*g33;
        }}
      }
      restoreSlice2(i2,gs[0],g112);
      restoreSlice2(i2,gs[1],g122);
      restoreSlice2(i2,gs[2],g132);
      restoreSlice2(i2,gs[3],g222);
      restoreSlice2(i2,gs[4],g232);
      restoreSlice2(i2,gs[5],g332);
    }});
  }

  private float[][] computeTensors(float phi, Sampling st) {
    int nt = st.getCount();
    float[][] ts = new float[6][nt];
    for (int it=0; it<nt; ++it) {
      float thi = (float)st.getValue(it);
  ...
  private static float[][][] semblanceFromNumDen(
    final float[][][] sn, final float[][][] sd) {
    final int n3 = sn.length;
    final int n2 = sn[0].length;
    final int n1 = sn[0][0].length;
    final float[][][] sem = new float[n3][n2][n1];
    loop(n3,new LoopInt() {
    public void compute(int i3) {
      sem[i3] = semblanceFromNumDen(sn[i3],sd[i3]);
    }});
    return sem;
  }
