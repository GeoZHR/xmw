...
public class FaultScannerF {
   ...
   */
  public FaultScannerF(double sigmaPhi, double sigmaTheta) {
    _sigmaPhi = sigmaPhi;
  ...
  public float[][][][] scanF(
      ...
      float[][][] p2, float[][][] p3, float[][][] g) {
    float[][][][] snd = semblanceNumDen(p2,p3,g);
    return scanF(phiSampling,thetaSampling,snd);
  }
  ...
  private float[][][][] scanF(
    ...
    float[][][][] snd) 
  {
    float spt = (float)(_sigmaPhi/_sigmaTheta);
    ...
    final float[][][] au = fillfloat(0.0001f,n1,n2,n3);
    final float[][][] av = fillfloat(1.0000f,n1,n2,n3);
    final float[][][] aw = fillfloat(1.0000f,n1,n2,n3);
    ...
    for (int ip=0; ip<np; ++ip) {
      ...
      for (int it=0; it<nt; ++it) {
        final float tpi = (float)thetaSampling.getValue(it);
        float[] us = faultNormalVectorFromStrikeAndDip(phi,tpi);
        float[] ws = faultDipVectorFromStrikeAndDip(phi,tpi);
        ...
        public void compute(int i3) {
          for (int i2=0; i2<n2; ++i2) {
            ...
            for (int i1=0; i1<n1; ++i1) {
              ...
              sri = sri*sri; // semblance^2
              sri = sri*sri; // semblance^4
              sri = sri*sri; // semblance^8
              float fi = 1.0f-sri;
              if (fi>f32[i1]) {
                f32[i1] = fi;
                p32[i1] = phi;
