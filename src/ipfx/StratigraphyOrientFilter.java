...
public class StratigraphyOrientFilter {
   ...
   */
  public StratigraphyOrientFilter(double sigma1, double sigma2) {
   ...
   */
  public StratigraphyOrientFilter(double sigma1, double sigma2, double sigma3) {
  ...
  public float[][][] applyForLinear(
    float sigma, EigenTensors3 et, float[][][] gx) 
  {
    int n3 = gx.length;
    int n2 = gx[0].length;
    ...
    //computeGradientsX(u1,u2,u3,p2,p3,gx,g1,g2,g3);
    RecursiveGaussianFilter rgf = new RecursiveGaussianFilter(1.0);
    rgf.apply100(gx,g1);
    rgf.apply010(gx,g2);
    rgf.apply001(gx,g3);
    computeGradientProductsX(sigma,et,g1,g2,g3,g11,g12,g13,g22,g23,g33);
    solveEigenProblemsX(g11,g12,g13,g22,g23,g33,null,null,null,null,el);
    return el;
  }
  ...
  public void computeGradientsX(
    final float[][][] u1, final float[][][] u2, final float[][][] u3,
    final float[][][] p2, final float[][][] p3, final float[][][] fx, 
    final float[][][] g1, final float[][][] g2, final float[][][] g3) {
    final int n3 = p2.length;
    final int n2 = p2[0].length;
    final int n1 = p2[0][0].length;
    ...
    for (int i2=0; i2<n2; ++i2) {
    for (int i1=0; i1<n1; ++i1) {
      float x1 = i1+u1[i3][i2][i1];
      float x2 = i2+u2[i3][i2][i1];
      float x3 = i3+u3[i3][i2][i1];
      float fp = si.interpolate(s1,s2,s3,fx,x1,x2,x3);
      g1[i3][i2][i1] = fp-fx[i3][i2][i1];
    }}}
    // compute g2
    ...
    public void compute(int i2) {
      ...
      for (int i1=0; i1<n1; ++i1)
        g3[n3-1][i2][i1] = g3[n3-2][i2][i1];
    }});
  }

  private void computeGradientProductsX(
    float sigma,
  ...
  {
    final int n3 = g2.length;
    final int n2 = g2[0].length;
    final int n1 = g2[0][0].length;
    Parallel.loop(n3,new Parallel.LoopInt() {
      public void compute(int i3) {
        for (int i2=0; i2<n2; ++i2) {
          float[] g1i = g1[i3][i2];
    ...
    lsf.apply(et,c,g11,g11);
    lsf.apply(et,c,g12,g12);
    lsf.apply(et,c,g13,g13);
    lsf.apply(et,c,g22,g22);
    lsf.apply(et,c,g23,g23);
    lsf.apply(et,c,g33,g33);
  }
  ...
  private void solveEigenProblemsX(
    ...
    final float[][][] w2, final float[][][] w3, final float[][][] el) 
  {
    // Compute eigenvectors, eigenvalues, and outputs that depend on them.
    final int n3 = g22.length;
    final int n2 = g22[0].length;
    final int n1 = g22[0][0].length;
    Parallel.loop(n3,new Parallel.LoopInt() {
      public void compute(int i3) {
        for (int i2=0; i2<n2; ++i2) {
          for (int i1=0; i1<n1; ++i1) {
