...
public class FaultScannerXX {
   ...
   */
  public FaultScannerXX(double sigmaPhi, double sigmaTheta) {
    _sigmaPhi = sigmaPhi;
  ...
  public static float[][][] padder(int m1, int m2, int m3, float[][][] f) {
    int n3 = f.length;
    int n2 = f[0].length;
    int n1 = f[0][0].length;
    int p1 = n1+2*m1;
    int p2 = n2+2*m2;
    int p3 = n3+2*m3;
    float[][][] fp = new float[p3][p2][p1];
    for (int i3=0; i3<n3; ++i3) {
    for (int i2=0; i2<n2; ++i2) {
    for (int i1=0; i1<n1; ++i1) {
      fp[i3+m3][i2+m2][i1+m1] = f[i3][i2][i1];
    }}}

    for (int i3=0; i3<p3; ++i3) {
    for (int i2=0; i2<p2; ++i2) {
    for (int i1=0; i1<=m1; ++i1) {
      fp[i3][i2][i1] = fp[i3][i2][m1-1];
    }}}
    for (int i3=0; i3<p3; ++i3) {
    for (int i2=0; i2<p2; ++i2) {
    for (int i1=n1+m1; i1<p1; ++i1) {
      fp[i3][i2][i1] = fp[i3][i2][n1+m1-1];
    }}}

    for (int i3=0; i3<p3; ++i3) {
    for (int i2=0; i2<=m2; ++i2) {
    for (int i1=0; i1<n1; ++i1) {
      fp[i3][i2][i1] = fp[i3][m2-1][i1];
    }}}
    for (int i3=0; i3<p3; ++i3) {
    for (int i2=n2+m2; i2<p2; ++i2) {
    for (int i1=0; i1<p1; ++i1) {
      fp[i3][i2][i1] = fp[i3][n2+m2-1][i1];
    }}}

    for (int i3=0; i3<=m3; ++i3) {
    for (int i2=0; i2<p2; ++i2) {
    for (int i1=0; i1<n1; ++i1) {
      fp[i3][i2][i1] = fp[m3-1][i2][i1];
    }}}
    for (int i3=n3+m3; i3<p3; ++i3) {
    for (int i2=0; i2<p2; ++i2) {
    for (int i1=0; i1<p1; ++i1) {
      fp[i3][i2][i1] = fp[n3+m3-1][i2][i1];
    }}}
    return fp;
  }
  ...
  public float[][][][] scan(
      ...
      double thetaMin, double thetaMax, float[][][] ep){
    Sampling sp = makePhiSampling(phiMin,phiMax);
    Sampling st = makeThetaSampling(thetaMin,thetaMax);
    return scan(sp,st,ep);
  }

  public float[][][][] scan(
      double phiMin, double phiMax,
      double thetaMin, double thetaMax, float[][][] p2, 
      float[][][] p3, float[][][] gx){
    Sampling sp = makePhiSampling(phiMin,phiMax);
    Sampling st = makeThetaSampling(thetaMin,thetaMax);
    float[][][][] snd = semblanceNumDen(p2,p3,gx);
    float[][][] ep = semblanceFromNumDen(snd[0],snd[1]);
    ep = pow(ep,8);
    ep = sub(ep,min(ep));
    ep = div(ep,max(ep));
    ep = sub(1.0f,ep);
  ...
  private static float[][][] semblanceFromNumDen(
    ...
    float[][][] sr = new float[n3][n2][n1];
    for (int i3=0; i3<n3; ++i3) {
      sr[i3]=semblanceFromNumDen(sn[i3],sd[i3]);
    }
    return sr;
  }
  ...
  public static float[][][] smooth(
    ...
    d.setEigenvalues(0.001f,0.001f,1.00f);
  ...
  private float[][][][] scan(
    ...
    for (int ip=0; ip<np; ++ip) {
      ...
      float[][][] rep = r.rotate(ep);
      smooth2(rep);
      float[][][][] rftp = scanTheta(thetaSampling,rep);
      rep = null; // enable gc to collect this large array
  ...
  private void smooth2(final float[][][] ep) {
    final int n1 = n1(ep), n2 = n2(ep), n3 = n3(ep);
    final RecursiveExponentialFilter ref = makeRef(_sigmaPhi);
    loop(n3,new LoopInt() {
    public void compute(int i3) {
      float[][] s3 = extractSlice3(i3,ep);
      if (s3!=null) {
        ref.apply2(s3,s3); 
        restoreSlice3(i3,ep,s3);
      }
    }});
  }
