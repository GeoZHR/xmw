...
def goScan():
  ...
  if not plotOnly:
    ...
    writeImageM(flfile,fl)
    writeImageM(ftfile,ft)
  else:
    fl = readImageM(n1,n2,flfile)
    ft = readImageM(n1,n2,ftfile)
...
def goFaultCurve():
  ...
  FaultCurve.getFlImage(200,cs,flt)
  plot2(s1,s2,gx,g=flt,cmin=0.2,cmax=1,cmap=jetFillExceptMin(1.0))
  writeImageM(fltfile,flt)
  '''
  k=0
  for ci in cs:
