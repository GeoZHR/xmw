...
seismicDir = "../../../data/seis/pik/fault/2d/"
fxfile = "tp73"
fxfile = "fxnwc"
fxfile = "clyde200"
fxfile = "gx238"
...
n1,n2 = 421,421
n1,n2 = 651,601  #fxnwc
n1,n2 = 400,801  #clyde200
n1,n2 = 300,1200 #gx238
...
def goSlopeVector():
  ...
  el = sub(1,el)
  foe = FaultOrientEstimator(40,20)
  et = foe.thin(0.1,el)
  #et = zerofloat(n1,n2)
  '''
  zero(el)
  for i1 in range(1,n1-1,5):
    el[i1][i1] = 1
    el[i1+80][i1] = 1
    el[round(i1*0.5)][n1-i1] = 1
    el[i1][200] = 1
  '''
  foe.apply(20,10,et,v1,v2)
  #plot(el,v1=v1,v2=v2,cmin=0.01,cmax=1,cint=1.0,label="Amplitude",png="fxv")
  au = fillfloat(0.01,n1,n2)
  av = fillfloat(1.0,n1,n2)
  es = fillfloat(0.0,n1,n2)
  lsf = LocalSmoothingFilter()
  etl = EigenTensors2(v1,v2,au,av)
  rgf = RecursiveGaussianFilter(1)
  rgf.apply00(et,es)
  lsf.apply(etl,200,es,es)
  es = sub(es,min(es))
  es = div(es,max(es))
  flt = foe.thin(0.1,es)
  #flt = pow(flt,0.5)
  ...
  sn = zerofloat(n1,n2)
  sd = zerofloat(n1,n2)
  lsf.apply(ets,4,fx,sn) 
  sn = pow(sn,2)
  fs = pow(fx,2)
  lsf.apply(ets,4,fs,sd) 
  lsf.applySmoothS(sn,sn)
  lsf.applySmoothS(sd,sd)
  lsf.apply(etl,400,sn,sn) 
  lsf.apply(etl,400,sd,sd) 
  fl = div(sn,sd)
  fl = pow(fl,1)
  fl = sub(1,fl)
  ft = foe.thin(0.15,fl)
