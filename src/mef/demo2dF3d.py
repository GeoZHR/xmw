...
"""
from utils2 import *
setupForSubset("f3d2d")
setupForSubset("clyde2d")
s1,s2,s3= getSamplings()
n1,n2= s1.count,s2.count

# Names and descriptions of image files used below.
gxfile  = "f3d75s" # migrated image 
gxfile  = "gx30" # migrated image 
gffile  = "gf"
...
def goScanC():
  ...
  sig1,sig2,smooth=16.0,2.0,2.0
  fls = fs.scanC(minTheta,maxTheta,sig1,sig2,smooth,gx)
  nt = len(fls)
  gs = zerofloat(n1,n2,nt)
  flc = zerofloat(n1,n2)
  for it in range(nt):
    gs[it] = copy(gx)
  plot2(s1,s2,gx)
  plot3(gs,fls,cmin=0.001,cmax=1)
  '''
  ft = zerofloat(nt)
  for it in range(nt):
    ft[it] = fls[it][775][230]
  fts = fs.parabolicFit(minTheta,maxTheta,ft)
  plot1(ft)
  plot1(fts)
  '''
  flr,ftr = fs.faultLikeFit(minTheta,maxTheta,fls)
  plot2(s1,s2,gx,g=flr,cmin=0.20,cmax=1,cmap=jetRamp(1.0),
      label="Fault likelihood improved")
  flt,ftt = fs.thin([flr,ftr])
...
def goFaultCurve():
  ...
  ets = lof.applyForTensors(fl)
  rgf1 = RecursiveGaussianFilter(1)
  rgf1.applyX0(fl,fl)
  ets.setEigenvalues(0.5,1.0)
  lsf = LocalSmoothingFilter()
  lsf.apply(ets,20,fl,fl)
  '''
  rgf1 = RecursiveGaussianFilter(1)
  ...
  writeImage(fltfile,flt)
  plot2(s1,s2,gx,g=flt,cmin=0.0,cmax=1,cmap=jetFillExceptMin(1.0))
  '''
  k=0
  for ci in cs:
...
def goFaultThrow():
  ...
  plot2(s1,s2,gx,g=mul(fst,1),cmin=0.1,cmax=25,
        cmap=jetFillExceptMin(1.0),label="Fault throw (ms)")
  plot2(s1,s2,gx,g=mul(fs1,1),cmin=0.1,cmax=25,
        cmap=jetFillExceptMin(1.0),label="vertical",)
  plot2(s1,s2,gx,g=mul(fs2,1),cmin=0.1,cmax=2,
        cmap=jetFillExceptMin(1.0),label="horizontal")
  '''
  smark = -999.999
