...
public class Flattener3 {
  ...
  public Mappings getMappingsFromSlopes(
    Sampling s1, Sampling s2, Sampling s3,
    float[][][] p2, float[][][] p3, float[][][] ep, float[][][] r) 
  {
    // Sampling parameters.
    final int n1 = s1.getCount();
    final int n2 = s2.getCount();
  ...
  */
  ...
  public void computeShifts(
    float[][][] p2, float[][][] p3, float[][][] ep, int[][][] uc, float[][][] r) 
  {
    // Compute shifts r(x1,x2,x3), in samples.
    int n3 = p2.length;
    int n2 = p2[0].length;
    ...
    VecArrayFloat3 vb = new VecArrayFloat3(b);
    VecArrayFloat3 vr = new VecArrayFloat3(r);
    float[][][] w1 = setWeightsFromUnconformities(ep,uc);
    Smoother3 smoother3 = new Smoother3(n1,n2,n3,_sigma1,_sigma2,_sigma3,ep,ep);
  ...
  public float[][][] setWeightsFromUnconformities(float[][][] wp, int[][][] uc) {
    int n3 = wp.length;
    int n2 = wp[0].length;
    ...
    if (uc==null) {
      ...
      int nc = uc[0].length;
      float[][][] w1 = fillfloat(0.005f,n1,n2,n3);
      for (int ic=0; ic<nc; ++ic) {
        ...
        for (int ip=0; ip<np; ++ip) {
          ...
          wp[i3][i2][i1] = 0.00f; 
          w1[i3][i2][i1] = 0.00f;
          if(i1m1>=0) {wp[i3][i2][i1m1] = 0.00f; w1[i3][i2][i1m1] = 0.005f;}
          if(i1p1<n1) {wp[i3][i2][i1p1] = 0.00f; w1[i3][i2][i1p1] = 0.005f;}
        }
      }
      extend(w1);
      extend(wp);
  ...
  private static class Smoother3 {
    public Smoother3(
      ...
      float sigma1, float sigma2, float sigma3, float[][][] epl, float[][][] epv) 
    {
      _sigma1 = sigma1;
      _sigma2 = sigma2;
      _sigma3 = sigma3;
    ...
    public void apply(float[][][] x) {
      smooth3(_sigma3,_epl,x);
      smooth2(_sigma2,_epl,x);
      smooth1(_sigma1,_epv,x);
      //removeAverage(x);
      //zero1(x);
    }
    ...
    public void testSpd() {
      // symmetric: y'Ax = x'(A'y) = x'Ay
      // positive-semidefinite: x'Ax >= 0
      int n1 = _epl[0][0].length;
      int n2 = _epl[0].length;
      int n3 = _epl.length;
      float[][][] x = sub(randfloat(n1,n2,n3),0.5f);
      float[][][] y = sub(randfloat(n1,n2,n3),0.5f);
      float[][][] ax = copy(x);
