...
public class LaplacianSmoother {
  ...
  public LaplacianSmoother (double sigma, int niter) {
    _niter = niter;
    _scale = (float)sigma*((float)sigma+1.0f)/6.0f;
    _lsf = new LocalSmoothingFilter(_small,_niter,_ldk);
  }
    ...
    Direction2 d, EigenTensors2 t, float[][] f, float[][] g) 
  {
    if (_scale==0.0f) {
      ...
      //_lsf.applySmoothS(f,g);
      //_lsf.applySmoothL(_kmax,f,g);
