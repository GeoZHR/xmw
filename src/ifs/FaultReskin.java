...
public class FaultReskin {
  ...
  public void checkNabors(final FaultCell[] cells) {
    final int nc = cells.length;
    final FaultCellGrid cellGrid = new FaultCellGrid(cells);
    Parallel.loop(nc, new Parallel.LoopInt(){
    public void compute(int ic){
      FaultCell ca = findNaborAbove(cellGrid,cells[ic]);
      FaultCell cb = findNaborBelow(cellGrid,cells[ic]);
      FaultCell cl = findNaborLeft (cellGrid,cells[ic]);
      FaultCell cr = findNaborRight(cellGrid,cells[ic]);
      if(ca!=null){cells[ic].ca=ca;}
      if(cb!=null){cells[ic].cb=cb;}
      if(cl!=null){cells[ic].cl=cl;}
      if(cr!=null){cells[ic].cr=cr;}
      if(cells[ic].ca!=null&&cells[ic].cb!=null&&
         cells[ic].cl!=null&&cells[ic].cr!=null) {
         cells[ic].setInterp(false);
      }
    }});
  }

  public void addNaborAbove(FaultCell cell, FaultCell[] cells) {
    FaultCellGrid cellGrid = new FaultCellGrid(cells);
    FaultCell ca = findNaborAbove(cellGrid,cell);
    if(ca!=null){cell.ca=ca;}
  }

  public void addNaborBelow(FaultCell cell, FaultCell[] cells) {
    FaultCellGrid cellGrid = new FaultCellGrid(cells);
    FaultCell cb = findNaborBelow(cellGrid,cell);
    if(cb!=null){cell.cb=cb;}
  }

  public void addNaborLeft(FaultCell cell, FaultCell[] cells) {
    FaultCellGrid cellGrid = new FaultCellGrid(cells);
    FaultCell cl = findNaborLeft(cellGrid,cell);
    if(cl!=null){cell.cl=cl;}
  }

  public void addNaborRight(FaultCell cell, FaultCell[] cells) {
    FaultCellGrid cellGrid = new FaultCellGrid(cells);
    FaultCell cr = findNaborRight(cellGrid,cell);
    if(cr!=null){cell.cr=cr;}
  }
