...
/**
 * Description:
 * Primitive class for the complex number (uses floats for real and imaginary)
 * Includes overloaded complex operation methods for 
 *     addition					adds a float or a complex
 *     subtraction 			subtracts a float or a complex
 *     multiplication		multiplies a float or a complex
 *     division 				divides a float or a complex
 *     conjugate				negates the imaginary
 *     absolute value
 *     exponentiation
 *     norm
 *     
...
public class Complex {
  ...
  public float abs() {
    if ( _r == 0 ) return Math.abs( _i );
    if ( _i == 0 ) return Math.abs( _r );
