 ...
 * Description:
 * <P/>
 * An arbitrary length Fast Fourier Transform (FFT) which is optimized for factors
 * of 2, 3, 4, and 5.  The computation is a simple Cooley-Tukey formulation.
 * Method nfft will return the next largest number which is purely can be factored 
 * by 2, 3, 5 should the user wish to speed up the solution.
 * The table of sines and cosines for the time series length is pre-computed for
...
public final class RealFFT {
  ...
  static public int nfft( int n ) {
    ...
    while ( next ) {
      ...
      if ( n%2 == 0 ) {
        n /= 2;  if ( n == 1 ) next = false;
      }

      // Test for factor of 3
      else if ( n%3 == 0) {
        n /= 3;  if ( n == 1 ) next = false;
      }
      
      // Test for factor of 5
      else if ( n%5 == 0) {
        n /= 5;  if ( n == 1 ) next = false;
      }
  ...
  private void checkTable() {
    if ( _table == null ) {
      ...
      if ( _table.length != _nf ) computeTable();
    }
  }
...
//      }
      ...
      if ( lnew == _nfft2 ) nextFactor = false;
      lold = lnew;
    }
...
//  For forward fft apply cosine/sines to result
  ...
  private int factor4( int lold, int dir ) {
    ...
    for (int i = 0; i < lold; i+=2) {
      ...
      for (int k = 1; k <= ns; k+=lold) {
        ...
        int k2 = k1 + lold;  
        int k3 = k2 + lold;
        int j0 = k  + i;   int j0m1 = j0 - 1;
        int j1 = j0 + ns;  int j1m1 = j1 - 1;
        int j2 = j1 + ns;  int j2m1 = j2 - 1;
        int j3 = j2 + ns;  int j3m1 = j3 - 1;
        if ( i != 0 ) {
          tr1 = c1*_a[j1m1] - s1*_a[j1];  ti1 = c1*_a[j1]  + s1*_a[j1m1];
          tr2 = c2*_a[j2m1] - s2*_a[j2];  ti2 = c2*_a[j2]  + s2*_a[j2m1];
          tr3 = c3*_a[j3m1] - s3*_a[j3];  ti3 = c3*_a[j3]  + s3*_a[j3m1];
          tr0 = _a[j0m1] + tr2;           ti0 = _a[j0]  + ti2;
          tr2 = _a[j0m1] - tr2;           ti2 = _a[j0]  - ti2;
          ar  = tr1 + tr3;               ai  = ti1 + ti3;
          tr3 = tr1 - tr3;               ti3 = ti1 - ti3;
        }
        else {
          tr0 = _a[j0m1] + _a[j2m1];  ti0 = _a[j0] + _a[j2];
          tr2 = _a[j0m1] - _a[j2m1];  ti2 = _a[j0] - _a[j2];
          ar  = _a[j1m1] + _a[j3m1];  ai  = _a[j1] + _a[j3];
          tr3 = _a[j1m1] - _a[j3m1];  ti3 = _a[j1] - _a[j3];
        }
        if ( dir < 0 ) { tr3 = -tr3;  ti3 = -ti3; }
        _aw[k0]   = tr0 + ar;
        _aw[k0+1] = ti0 + ai;
        _aw[k1++] = tr2 + ti3;
        _aw[k1]   = ti2 - tr3;
  ...
  private int factor3( int lold, int dir ) {
    float wc = -0.5000000f;
    float ws = -0.8660300f;  if ( dir < 0 ) ws = -ws; 
    ...
    for (int i = 0; i < lold; i+=2) {
      ...
      for (int k = 1; k <= ns; k+=lold) {
        ...
        if ( i != 0 ) {
          tr1 = c1*_a[j1m1] - s1*_a[j1];  ti1 = c1*_a[j1] + s1*_a[j1m1];
          tr2 = c2*_a[j2m1] - s2*_a[j2];  ti2 = c2*_a[j2] + s2*_a[j2m1];
          tr0 = tr1 + tr2;  ti0 = ti1 + ti2;
          tr1 = tr1 - tr2;  ti1 = ti1 - ti2;
        }
        else {
          tr0 = _a[j1m1] + _a[j2m1];  ti0 = _a[j1] + _a[j2];
          tr1 = _a[j1m1] - _a[j2m1];  ti1 = _a[j1] - _a[j2];
        }
        tr2 = tr0*wc;  ti2 = ti0*wc;
        ar  = ti1*ws;  ai  = tr1*ws;
        _aw[k0]   = _a[j0m1] + tr0;
        _aw[k0+1] = _a[j0]   + ti0;
        tr0 = _a[j0m1] + tr2;  ti0 = _a[j0] + ti2;
        _aw[k1++] = tr0 - ar;  _aw[k1] = ti0 + ai;
        _aw[k2++] = tr0 + ar;  _aw[k2] = ti0 - ai;
        k0 += lnew;
      }
    }
    return lnew;
  ...
  private int factorN( int n, int lold, int dir ) {
    ...
    for (int l = ll; l < (lnew+lold)/2; l+=lold) {
      ...
      for (int k = 0; k < ns2; k+=ns) {
        int kk = _nfft2 - k - 2;
        if ( k != 0 ) {
          nw += ndel;  if ( nw > _nfft2 ) nw -= _nfft2;
          int nw1 = nw + 1;
          if ( nw <= _nfft ) {
            c1 = _table[nw];
            s1 = _table[nw1];
        ...
        for (int j = 0; j < ns; j+=lold) {
          ...
          for (int i = 1; i <= lold; i+=2) {
            int k1 = i  + ll;  int k2 = k1 + lc + 1;
            int j3 = i  + j1;  int j3m1 = j3 - 1;
            int j4 = i  + j2;  int j4p1 = j4 + 1;        
            if ( k <= 0 ) {
              _aw[k1-1] = _a[j3m1];
              _aw[k1]   = _a[j3];
              _aw[k2++] = _a[j3m1];
  ...
  public static void main( String[] args) {
    ...
    float t  = 0.0f;
    float dt = 0.002f;
    float p2 = (float)(2.0*Math.PI);
    for (int i = 0; i < n; i++) {
      realInput[i] = (float) (Math.sin(p2*f1*t) + Math.sin(p2*f2*t) + Math.sin(p2*f3*t) + Math.sin(p2*f4*t));
      t += dt;
    }
    ...
    System.out.println(" fft run time: "+(System.nanoTime()-start));
