...
def goReskin():
  ...
  fls = like(fl)
  FaultSkin.getLikelihood(skins,fls)
  fls = pow(fls,0.6)
  sks1 = []
  sks2 = []
  idex = 0
  for ski in skins:
    ...
    fpa = fp/ski.size()
    print fpa
    if fpa>150 and fpa<200:
      sks1.append(ski)
    if fpa>100 and fpa<150 and idex != 17 and idex!=19:
      sks2.append(ski)
      #plot3(gx,skins=[ski],clab=str(idex))
    idex = idex+1
  #plot3(fw)
  plot3(gx,skins=sks1,png="skins150200")
  plot3(gx,skins=sks2,png="skins100150")
  #plot3(gx,skins=skins)#,png="skins")
  #plot3(gx,fls,cmin=0.01,cmax=1.0,cmap=jetFillExceptMin(1.0),
  #      clab="Fault likelihood")#,png="flts")
...
def goSlip():
  ...
  mark = -100
  fth = fillfloat(mark,n1,n2,n3)
  FaultSkin.getThrow(mark,skins,fth)
  d1 = 0.002
  mul(fth,d1*1000,fth)
  plot3(gx,fth,cmin=-0.1,cmax=max(fth)-70,cmap=jetFillExceptMin(1.0),
        clab="Fault throw (ms)",png="fth")

  plot3(gx,skins=skins,png="skinsfl")
  plot3(gx,skins=skins,smax=15.0,png="skinss1")
...
def goUnfaultS():
  if not plotOnly:
    ...
    gx = readImage(gxfile)
    fw = readImage(fwsfile)
    t1 = readImage(frs1file)
    t2 = readImage(frs2file)
    t3 = readImage(frs3file)
  gx = gain(gx)
  fw = gain(fw)
  plot3(gx,png="gx")
  plot3(fw,png="fw")
  d1 = 0.002
  mul(t1,d1*1000,t1)
  plot3(gx,t1,cmin=min(t1)+30,cmax=max(t1)-9,cmap=jetFill(0.3),
        clab="Vertical shifts (ms)",png="uft1")
