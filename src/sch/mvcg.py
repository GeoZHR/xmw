...
def main(args):
  ...
  for skin in skins:
    ...
    mul(ux,0.2,ux)
    mul(vx,0.5,vx)
    em.applyForOneEllipsoid(cx,ux,vx,wx)
  '''
  cx=zerofloat(3)
  ux=zerofloat(3)
  vx=zerofloat(3)
  ...
  smoother = Smoother3(4.0,2.0,2.0,wp)
  gx = readImage(gxfile)
  r1 = readImage("r94")
  c1 = readImage("c85")
  smoother.applyOriginal(r1)
  smoother.applyOriginal(c1)
  slices=[99,197,184]
  plot3(gx,r1,cmin=-8,cmax=8,cmap=jetFill(0.3),slices=slices,
      clab="Vertical shift (samples)")
  plot3(gx,c1,cmin=-8,cmax=8,cmap=jetFill(0.3),slices=slices,
      clab="Vertical shift (samples)")
...
def goFrames():
  ...
  '''
  skins=[sk[0],sk[1],sk[3],sk[4],sk[5],sk[6],sk[7],sk[8],sk[9],sk[10],
         sk[11],sk[12],sk[13],sk[14],sk[15],sk[16],sk[17],sk[18],
         sk[19],sk[20],sk[21],sk[22]]
  '''
  wp = fillfloat(1.0,n1,n2,n3)
  ...
  for cell in skin:
    cell.setFl(0.5)
  plot3(gx,c1,skins=[skin],cmin=-6,cmax=6,cmap=jetFill(0.3),
          slices=slices,png=c1file+"skin")
  '''
  for i in range(0,86,5):
  #for i in range(1):
    m1file = "m"+str(i)
    m1 = readImage(m1file)
    smoother.applyOriginal(m1)
    plot3(gx,m1,cmin=-6,cmax=6,cmap=jetFill(0.3),slices=slices,png=m1file)
    if i==0:
      skin = sk[2]
      for cell in skin:
        cell.setFl(0.3)
      plot3(gx,m1,skins=[skin],cmin=-6,cmax=6,cmap=jetFill(0.3),
            slices=slices,png=m1file+"skin")
...
def goUnfaultS():
  ...
  t2 = zerofloat(n1,n2,n3)
  t3 = zerofloat(n1,n2,n3)
  [r1,r2,r3] = uf.findShifts(sp,wp,[t1,t2,t3])
  plot3(gx,r1,cmin=-8,cmax=8,cmap=jetFill(0.3),slices=slices,
      clab="Vertical shift (samples)")
  writeImage("sht1",r1)
  writeImage("sht2",r2)
  ...
  s3 = copy(r3)
  mul(sp[3][0],10,sp[3][0])
  [c1,c2,c3] = uf.findShifts(sp,wp,[s1,s2,s3])
  plot3(gx,c1,cmin=-8,cmax=8,cmap=jetFill(0.3),slices=slices,
      clab="Vertical shift (samples)")
  '''
  writeImage("shc1",c1)
  ...
  sp = fsc.screenPoints(wp)
  mul(sp[3][0],10,sp[3][0])
  [m1,m2,m3] = uf.findShifts(sp,wp,[k1,k2,k3])
  plot3(gx,m1,cmin=-8,cmax=8,cmap=jetFill(0.3),slices=slices,
      clab="Vertical shift (samples)")
  writeImage("shm1",m1)
  writeImage("shm2",m2)
  writeImage("shm3",m3)
