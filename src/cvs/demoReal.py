...
#pngDir = None
pngDir = "../../../png/"
seismicDir = "../../../data/seis/pnz/pnz/"
s1 = Sampling(200,1.0,0)
s2 = Sampling(879,1.0,0)
s3 = Sampling(752,1.0,0)
...
def slopes():
  ...
  lsf = LocalSlopeFinder(sigma1,sigma2,10) 
...
def multipleControlPoints():
  k11 = [ 74, 80, 76,74,  81, 67, 49, 48, 56, 47, 84, 78,105, 78]
  k12 = [508,472,423,602,578,455,145,132, 39, 39,435,529,640,537]
  k13 = [401,491,432,208,375,209,449,196,259,494,347,347,395,209]
  f = readImage(gxfile)
  p2 = readImage(p2file)
  p3 = readImage(p3file)
  ep = readImage(epfile)
...
def rgbFromAmplitude(f,h,r,g,b):
  ...
  mp = ColorMap(aMin,aMax,ColorMap.GRAY)
...
def displayHorizon(pg,filename):
  ...
  ipg = addImageToWorld(world,f,cmap=ColorMap.GRAY,cmin=-2.0,cmax=2.0)
  ipg.setSlices(k1,k2,k3)
