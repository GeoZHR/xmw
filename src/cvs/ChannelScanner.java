...
public class ChannelScanner {
   ...
   */
  public ChannelScanner(double sigmaPhi, double sigmaTheta) {
    _sigmaPhi = sigmaPhi;
  ...
  * @param p3 slopes in the 3rd dimension.
  ...
  public static float[][][] smooth2(
    double sigma, float[][][] p2, float[][][] p3, float[][][] g) {
    int n1 = g[0][0].length;
    int n2 = g[0].length;
    int n3 = g.length;
    EigenTensors3 d = new EigenTensors3(n1,n2,n3,true);
    d.setEigenvalues(0.0001f,0.0001f,1.0000f);
    for (int i3=0; i3<n3; ++i3) {
      for (int i2=0; i2<n2; ++i2) {
        for (int i1=0; i1<n1; ++i1) {
          float p2i = p2[i3][i2][i1];
  ...
  private static float[][][] semblanceFromNumDen(
    ...
    for (int i3=0; i3<n3; ++i3) {
      s[i3] = semblanceFromNumDen(sn[i3],sd[i3]);
    }
    return s;
  }
