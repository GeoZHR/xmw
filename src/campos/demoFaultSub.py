...
setupForSubset("camposSub")
s1,s2,s3 = getSamplings()
n1,n2,n3 = s1.count,s2.count,s3.count
# Names and descriptions of image files used below.
gxfile  = "gxs" # input image (maybe after bilateral filtering)
p2file = "p2"
p3file = "p3"
gxpfile  = "gxp" # input image (maybe after bilateral filtering)
epfile  = "ep" # fault likelihood
smfile  = "sem" # fault likelihood
...
def goSemblance():
  print "compute semblance..."
  gx = readImage(gxfile)
  if not plotOnly:
    lof = LocalOrientFilter(64,4)
...
def goSkin():
  ...
  if not plotOnly:
    ...
    fp = readImage(fpfile)
    ft = readImage(ftfile)
    print "fault images loading done ..."
    fsk = FaultSkinner()
