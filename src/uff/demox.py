...
def goFlattenC():
  if not plotOnly:
    ...
    writeImage(fc1file,r1)
    writeImage(fc2file,r2)
    writeImage(fc3file,r3)
  else :
    gx = readImage(gxfile)
    fw = readImage(fwcfile)
    r1 = readImage(fc1file)
    r2 = readImage(fc2file)
    r3 = readImage(fc3file)
  plot3(gx)
  plot3(fw)
  '''
...
def goFlattenS():
  if not plotOnly:
    gx = readImage(gxfile)
    gs = readImage(gsxfile)
    fw = zerofloat(n1,n2,n3)
    gw = zerofloat(n1,n2,n3)
    fsk = readSkins(fskgood)
    fsl = readSkins(fslbase)
    fsc = FaultSlipConstraints(fsl)
    u1 = fillfloat(1.0,n1,n2,n3)
    u2 = fillfloat(0.0,n1,n2,n3)
    u3 = fillfloat(0.0,n1,n2,n3)
    ep = fillfloat(0.0,n1,n2,n3)
    sc = fillfloat(1.0,n1,n2,n3)
    fsc.setValuesOnFaults(0.0,fsk,sc)
    lofx = LocalOrientFilterX(2.0,1.0,1.0)
    #lofx.setLocalSmoothingFilter(sc)
    lofx.applyForNormalPlanar(gx,u1,u2,u3,ep)
    plot3(mul(u1,2.0))
    wp = pow(ep,2.0)
    ws = pow(ep,2.0)
