...
public class FlattenerRTX {
   ...
   */
  public FlattenerRTX(double sigma1, double sigma2) {
  ...
  public float[][][][] computeShifts(boolean unfaultOnly, 
    float[][][] cx, float[][][] cs, float[][][] ws, float[][][][] p) 
  {
    _cs = cs;
    int n3 = p[0].length;
    int n2 = p[0][0].length;
  ...
  */
  ...
  private static class A3 implements CgSolver.A {
    A3(float epsilon, float[][][] cs, float[][][][] p) {
      _epsilon = epsilon;
      _p = p;
      _cs = cs;
    }
  ...
  private static void makeRhs(
    final float[][][] cs, final float[][][][] p, final float[][][][] y)
  { 
    ...
    if(cs!=null){screenRhs(cs[0],cs[1],cs[2],cs[3][0],y);}
  }
