...
public class FlattenerRTM {
   ...
   */
  public FlattenerRTM(double sigma1, double sigma2) {
  ...
  */
    ...
    float [][][] fm, float[][][] cx, float[][][] cu, int[] cn) 
  {
    ...
    for (int is=0; is<ns; ++is) {
      ...
      int ia1 = round(ua1); if(ia1<0||ia1>m1){continue;}
      int ia2 = round(ua2); if(ia2<0||ia2>m2){continue;}
      int ia3 = round(ua3); if(ia3<0||ia3>m3){continue;}
      ...
      int ib1 = round(ub1); if(ib1<0||ib1>m1){continue;}
      int ib2 = round(ub2); if(ib2<0||ib2>m2){continue;}
      int ib3 = round(ub3); if(ib3<0||ib3>m3){continue;}
  ...
  private void updateConstraints(float[][][] cx, float[][][][] r, float[][][] cu) {
    int n3 = r[0].length;
    int n2 = r[0][0].length;
    ...
    for (int is=0; is<ns; ++is) {
      ...
      float s2 = cx[1][is][2];
      float s3 = cx[2][is][2];
      float s1 = 1.0f/cx[0][is][2];

      float r1 = b1-a1;
      float r2 = abs(r1*s2*s1);
      float r3 = abs(r1*s3*s1);
      float r1m,r1p,r2m,r2p,r3m,r3p;
      float scb = 4.0f;
      float scs = 1.0f;
      r1m = 0.0f; r1p = r1*scs;
      if(s2<0.f){r2m = -scs*r2; r2p = +scb*r2;}
      else      {r2m = -scb*r2; r2p = +scs*r2;}
      if(s3<0.f){r3m = -scs*r3; r3p = +scb*r3;}
      else      {r3m = -scb*r3; r3p = +scs*r3;}
      // find ua corresponding to xa;
      for (float d3=r3m; d3<=r3p; d3+=0.5f) {
        for (float d2=r2m; d2<=r2p; d2+=0.5f) {
          for (float d1=r1m; d1<=r1p; d1+=0.5f) {
            if (outsideA(s1,s2,s3,d1,d2,d3)){continue;}
            float u1 = a1+d1;
            float u2 = a2+d2;
            float u3 = a3+d3;
            int i1 = round(u1);
            int i2 = round(u2);
            int i3 = round(u3);
            if(i1<0||i1>m1){continue;}
            if(i2<0||i2>m2){continue;}
            if(i3<0||i3>m3){continue;}
            float x1 = i1-r[0][i3][i2][i1];
            float x2 = i2;//-r[1][i3][i2][i1];
            float x3 = i3;//-r[2][i3][i2][i1];
            float xa1 = x1-a1;
            float xa2 = x2-a2;
            float xa3 = x3-a3;
            float dax = xa1*xa1+xa2*xa2+xa3*xa3;
            if(dax<ds) {
              ds = dax;
              cu[0][is][0] = i1;
              cu[1][is][0] = i2;
      ...
      // find ub corresponding to xb;
      ds = 5000.f;
      r1p = 0.0f;
      r1m = -r1*scs;
      if(s2<0.f){r2m = -scb*r2; r2p = +scs*r2;}
      else      {r2m = -scs*r2; r2p = +scb*r2;}
      if(s3<0.f){r3m = -scb*r3; r3p = +scs*r3;}
      else      {r3m = -scs*r3; r3p = +scb*r3;}
      for (float d3=r3m; d3<=r3p; d3+=0.5f) {
        for (float d2=r2m; d2<=r2p; d2+=0.5f) {
          for (float d1=r1m; d1<=r1p; d1+=0.5f) {
            if (outsideB(s1,s2,s3,d1,d2,d3)){continue;}
            float u1 = b1+d1;
            float u2 = b2+d2;
            float u3 = b3+d3;
            int i1 = round(u1);
            ...
            float xb2 = x2-b2;
            float xb3 = x3-b3;
            float dbx = xb1*xb1+xb2*xb2+xb3*xb3;
            if(dbx<ds) {
              ds = dbx;
              cu[0][is][1] = i1;
              cu[1][is][1] = i2;
  ...
  private boolean outsideA(
    float s1, float s2, float s3, float d1, float d2, float d3) 
  {
    float dx2 = abs(d1*s2*s1);
    if (d2*s2>0 && abs(d2)>dx2) {return true;}
    float dx3 = abs(d1*s3*s1);
    if (d3*s3>0 && abs(d3)>dx3) {return true;}
    return false;
  }
