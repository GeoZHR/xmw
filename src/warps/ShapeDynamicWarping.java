 ...
 * @version 2017.12.29
 */
public class ShapeDynamicWarping {
   ...
   */
  public ShapeDynamicWarping(int shiftMin, int shiftMax, int d) {
  ...
  private int _d; // length of subsequence
  ...
  private void updateSmoothingFilters() {
    ...
    //_ref1.setEdges(edges);
    //_ref2.setEdges(edges);
    //_ref3.setEdges(edges);
  }
  ...
  private void computeErrors(float[] f, float[] g, float[][] e) {
    ...
    /*
    for (int i1=0; i1<n1; ++i1) {
      float[] fs = subsequence(_d,i1,f);
      for (int il=0; il<nl; ++il) {
        int j1 = i1+il+_lmin;
        float[] gs = subsequence(_d,j1,g);
        int nh = gs.length;
        float ei = 0f;
        for (int ih=0; ih<nh; ih++) {
          ei += error(fs[ih],gs[ih]);
        }
        e[i1][il] = ei/nh;
      }
    }
    */
    ...
    for (int i1=0; i1<n1; ++i1) {
      ...
      for (int il=illo,j1=i1+il+_lmin; il<ilhi; ++il,++j1) {
        float ei = 0f;
        float si = 0f;
        for (int di=-_d; di<=_d; di++) {
          int p1 = i1+di;
          int q1 = j1+di;
          if(p1>=0&&p1<n1&&q1>=0&&q1<n1) {
            si += 1f;
            ei += error(f[p1],g[q1]);
          }
        }
  ...
  private void computeErrorsX(float[] f, float[] g, float[][] e) {
    ...
    for (int i1=0; i1<n1; ++i1) {
      float[] fs = subsequence(_d,i1,f);
      float[] hf = getHog(0.1f,0.1f,10f,9,fs);
      for (int il=0; il<nl; ++il) {
        int j1 = i1+il+_lmin;
        float[] gs = subsequence(_d,j1,g);
        float[] hg = getHog(0.1f,0.1f,10f,9,gs);
        int nh = hg.length;
        float ei = 0f;
        for (int ih=0; ih<nh; ih++) {
          ei += error(hf[ih],hg[ih]);
        }
        e[i1][il] = ei/nh;
      }
    }
  ...
  private void computeErrorsC(float[] f, float[] g, float[][] e) {
    int nl = _nl;
    int n1 = f.length;
    ...
    LocalCorrelationFilter lcf = new LocalCorrelationFilter(tp,wd,_d);
    lcf.setInputs(f,g);
    for (int il=0; il<nl; ++il) {
      float[] c = new float[n1];
      lcf.correlate(il+_lmin,c);
      for (int i1=0; i1<n1; ++i1)
        e[i1][il] = c[i1];
    }
    sub(max(e),e,e);
  ...
  private float[] subsequence(int d, int k1, float[] fx) {
    int m1 = d*2+1;
    int n1 = fx.length;
    float[] fs = new float[m1];
    Random rd = new Random();
    for (int i1=0; i1<m1; i1++) {
      int j1 = i1+k1-d;
      if(j1<0||j1>=n1) j1 = rd.nextInt(n1);
      fs[i1] = fx[j1];
    }
    return fs;
  }
  ...
  private float[] getHog(
    float dx, float overlap, float sigma, int nb, float[] fx) {
    dx *= 2f;
    float da = 180/nb;
    float sigs = 2f*sigma*sigma;
    int n1 = fx.length;
    float[] gx = new float[n1];
    for (int i1=1; i1<n1-1; i1++)
      gx[i1] = fx[i1+1]-fx[i1-1];
    gx[0   ] = gx[1   ];
    gx[n1-1] = gx[n1-2];

    int m1 = round(n1/(2f-overlap));
    float[] hog = new float[nb*2];
    float pi = (float)(180f/(Math.PI));
    for (int i1=0; i1<m1; ++i1) {
      float ai = pi*atan2(gx[i1],dx);
      float mi = abs(gx[i1]/dx);//sqrt(gx[i1]*gx[i1]+dx*dx);
      for (int ib=0; ib<nb; ++ib) {
        float ci = -90+(ib+0.5f)*da;
        float di = ai-ci;
        hog[ib] += mi*exp(-di*di/sigs);
      }
    }
    for (int i1=round(m1-m1*overlap); i1<n1; ++i1) {
      float ai = pi*atan2(gx[i1],dx);
      float mi = abs(gx[i1]/dx);//sqrt(gx[i1]*gx[i1]+dx*dx);
      for (int ib=0; ib<nb; ++ib) {
        float ci = -90+(ib+0.5f)*da;
        float di = ai-ci;
        hog[ib+nb] += mi*exp(-di*di/sigs);
      }
    }
    return hog;
  }
