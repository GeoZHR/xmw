...
"""
from utils import *
setupForSubset("nwc2d")
s1,s2,s3= getSamplings()
n1,n2= s1.count,s2.count

# Names and descriptions of image files used below.
gxfile  = "gx268" # migrated image 
fltfile = "flt268" # fault likelihood thinned
epfile  = "ep" # eigenvalue-derived planarity
p2file  = "p2" # inline slopes
flfile  = "fl268" # fault likelihood
ftfile  = "ft268" # fault dip (theta)
...
def goFaultCurve():
  ...
  for ci in cs:
    ...
    for pi in ci:
      ...
      if(pi.getX2()>p2m):
        p2m=pi.getX2()
    if(p1m>360 and p2m<560):
      cc.append(ci)
  ftt = zerofloat(n1,n2)
  flt = zerofloat(n1,n2)
  FaultCurve.getFtImage(cc,ftt)
  FaultCurve.getFlImage(cc,flt)
  plot2(s1,s2,gx,g=flt,cmin=0.2,cmax=1,cmap=jetFillExceptMin(1.0))
  '''
...
def goFaultThrow():
  ...
  print min(fst)
  print max(fst)
  plot2(s1,s2,gx,g=fs1,cmin=0.1,cmax=maxThrow,
        cmap=jetFillExceptMin(1.0),label="Fault throw (samples)",png="slip1")
  plot2(s1,s2,gx,g=fs2,cmin=0.0,cmax=3,
        cmap=jetFillExceptMin(1.0),label="Fault heave (samples)",png="slip2")
  ...
  writeImageM("seisUnfaulted",gw)
  plot2(s1,s2,gx,label="Amplitude",png="seis")
  plot2(s1,s2,gw,label="Amplitude",png="seisUnfault")
  return cc
...
def plot2(s1,s2,f,g=None,cmin=None,cmax=None,cmap=None,label=None,png=None):
  ...
  panel = panel2Teapot()
  panel.setHInterval(40.0)
  panel.setVInterval(40.0)
  panel.setHLabel("Lateral position (samples)")
