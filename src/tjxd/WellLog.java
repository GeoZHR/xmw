...
public class WellLog {
  ...
  public long id; // unique id number
  public double xe,yn,ze; // easting, northing, elevation
  public int n; // number of samples
  public float[] zt; // depth for time-depth function
  public float[] tz; // time for time-depth function
  public float[] tvd; // time-depth function
  ...
  public float c2,c3; //coordinates of wellbores
  ...
  public void loadTimeDepth(String tdDirName) {
  }
  ...
  public static class Data {
     ...
     */
    public Data(String wlDirName, String dvtDirName) {
      File wlDir = new File(wlDirName);
      File dvtDir = new File(dvtDirName);
      ArrayList<String> wldup = new ArrayList<String>();
      load(wlDir,this,wldup);
      if (dvtDirName!=null)
        loadDvt(dvtDir, this);
    ...
    private Map<String,WellLog> _data = new HashMap<String,WellLog>();
    private static void load(
      File wlDir, WellLog.Data wldata, ArrayList<String> wldup)
    {
      Check.argument(wlDir.isDirectory(),wlDir+" is a directory");
      for (File file:wlDir.listFiles()) {
        if (file.isDirectory()) {
          load(file,wldata,wldup);
        } else if (file.isFile()) {
          WellLog log = WellLog.load(file);
          if (log!=null) {
            String fileName = file.getName().toString();
            log.name = fileName.replaceFirst("[.][^.]+$","");
            if (wldata.get(log.name)!=null) {
              wldup.add(log.name);
            } else {
              wldata.add(log);
            }
    ...
    private static void loadDvt(File dvtDir, WellLog.Data wldata) {
      Check.argument(dvtDir.isDirectory(),dvtDir+" is a directory");
      List<WellLog> logs = wldata.getAll();
      for (WellLog log : logs) {
        for (File file:dvtDir.listFiles()) {
          if (file.isDirectory()) {
            loadDvt(file, wldata);
          } else if (file.isFile()) {
            String fileName = file.getName().toString();
            String name = fileName.replaceFirst("[.][^.]+$","");
            if (name.equals(log.name)) {
              WellLog.loadDvt(file,log);
            }
          }
        }
      }
  ...
  public static WellLog load(File file) {
    ...
    int kz = 0, kv = -1, kd = -1, kg = -1, kp = -1;
    int index = 0;
    while (scanner.hasNextLine()) {
      String line = scanner.nextLine();
      if (line.length()>22) {
        String api1 = line.substring(0,3);
        System.out.println(api1);
        if (api1.equals("GR ")) {
          kg = index;
        } else if (api1.equals("AC ")) {
          kv = index;
        } else if (api1.equals("DEN")) {
          kd = index;
        }
      }
      ++index;
   ...
   * @return the well log; null, if the file format is not understood.
   */
  public static void loadDvt(File file, WellLog log) {
    ...
    // Read all well curve data as floats.
    FloatList zls = new FloatList();
    FloatList tls = new FloatList();
    FloatList tvd = new FloatList();
    scanner = new Scanner(adata);
    int n = 0;
    while (scanner.hasNextLine()) {
      String line = scanner.nextLine().trim();
      ...
      if (values.length==3){ 
        ...
        if(v1==-999.25f) continue;
        if(v2==-999.25f) continue;
        if(v3==-999.25f) continue;
        zls.add(v1);
        tvd.add(v2);
        tls.add(v3);
      }
    ...
    // set the time-depth relationship for seismic-well tying.
    log.n = n;
    log.zt = zls.trim();
    log.tz = tls.trim();
    log.tvd = tvd.trim();
    //convert ms to s if neccesary
    float tmax = max(log.tz);
    if(tmax>100f) {
      for (int i=0; i<n; ++i) { 
        log.tz[i] *= 0.001f;
      }
  ...
  public float[][] getSamplesX(String curve) {
    float[] fx = getCurve(curve);
    float[] zt = this.zt;
    float[] zs = this.z;
    float[] ts = this.tz;
    if (fx==null||ts==null)
      return null;
    FloatList fxl = new FloatList();
    FloatList x1l = new FloatList();
    int n = ts.length;
    for (int i=0; i<n; ++i) {
      float x1i = ts[i];
