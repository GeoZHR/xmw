...
public class HorizonExtraction {
    ...
    Sampling t1, int k2, int k3) 
  {
    ...
    float d1 = (float)_s1.getDelta();
    float f1 = (float)_s1.getFirst();
    float rgtMin = (float)u1s[0];
    float rgtMax = (float)u1s[n1s-1];
    //float rgtMin = 10;
    //float rgtMax = n1;
    ...
    for (int i1s=0; i1s<n1s; ++i1s) {
      ...
      int p = 0;
      for (int i3=0; i3<n3; ++i3) {
        float x3 = (float)_s3.getValue(i3);
        float x2 = (float)_s2.getValue(k2);
        float x1 = _x1[i3][k2][i1];
        if(x1>=n1) {continue;}
        surfs[k][p++] = x3;
        surfs[k][p++] = x2;
      ...
      p = 0;
      for (int i2=0; i2<n2; ++i2) {
        float x3 = (float)_s3.getValue(k3);
        float x2 = (float)_s2.getValue(i2);
        float x1 = _x1[k3][i2][i1];
        if(x1>=n1) {continue;}
        surfs[k][p++] = x3;
        surfs[k][p++] = x2;
  ...
  public float[][][] multipleHorizons(Sampling t1) {
    return multipleHorizons(t1,null);
  }
  ...
  public float[][][] multipleHorizons(Sampling t1, FaultSkin[] sks) {
    ...
    float umin = (float)u1s[0];
    float umax = (float)u1s[n1s-1];
    for (int i1s=0; i1s<n1s; ++i1s) {
      float u1i = (float) u1s[i1s];
      if(sks==null) {
        surfs[i1s] = singleHorizon(u1i,umin,umax);
      } else {
  ...
  public float[][] singleHorizon(float u1i, float umin, float umax) {
    int n3 = _u1.length;
    int n2 = _u1[0].length;
    int n1 = _u1[0][0].length;
    float d1 = (float)_s1.getDelta();
  ...
  public float[][][] horizonVolumeFromRgtx(
  ...
  public float[][][] horizonVolumeFromRgt(
    final Sampling s1, final float[][][] u1) 
  {
    cleanRGT(u1);
    final int n3 = u1.length;
