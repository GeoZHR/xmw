...
public class TensorsView extends TiledView {
  ...
  public void setEllipsesDisplayed(float[][] mask) {
    _mask = mask;
    updateTensorEllipsesX();
  }
  ...
  private void updateTensorEllipses() {
   ...
   // Ellipse polygon sampling.
    ...
    if (_mask!=null) {
      ...
      for (int i1=1; i1<n1-1; i1+=5) {
        if(_mask[i2][i1]==1f) {nm++;}
      }}
    }
    System.out.println("nm="+nm);
    ...
    if (_mask!=null) {
      for (int i2=1; i2<n2-1; ++i2) {
      int j2 = i2;
      double x2 = _s2.getValue(i2);
      for (int i1=1; i1<n1-1; i1+=5) {
        int j1 = i1;
        double x1 = _s1.getValue(i1);
        if(_mask[j2][j1]==1f) {
          _et.getEigenvalues(j1,j2,a);
          _et.getEigenvectorU(j1,j2,u);
          double u1 = u[0];
  ...
  private void updateTensorEllipsesX() {
    int n1 = _s1.getCount();
    int n2 = _s2.getCount();
    double d1 = _s1.getDelta();
    double d2 = _s2.getDelta();
   ...
   // Ellipse polygon sampling.
    ...
    int nm = 0;
    if (_mask!=null) {
      for (int i2=10; i2<n2-10; ++i2) {
      for (int i1=10; i1<n1-10; i1+=5) {
        if(_mask[i2][i1]==1f) {nm++;}
      }}
    }
    System.out.println("nm="+nm);
