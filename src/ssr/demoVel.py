...
def goFaultThrow():
  ...
  wp = sub(1,flt);
  gs = zerofloat(n1,n2)
  lsf.apply(ets,8,wp,gx,gs)
  fcr = FaultCorrelater(gs)
...
def goVelocity():
  ...
  ve.setTensors(et)
  ve.setSmoothness(150.0)
  ps = pow(pk,2)
  vi = ve.applyForVelocity(sm,mk,ps)
  vi = sqrt(vi)
  ...
  plot2(s1,s2,gx,g=vi,cmin=1.4,cmax=3.0,cmap=jetFill(1.0))
