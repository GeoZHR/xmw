...
public class RgtInterp3 {
  ...
  public void setBounds(float bv) {
    int n1 = _x1.length;
    float xmin = min(_x1);
    float xmax = max(_x1);
  ...
  public void setBounds(float[][][] fx) {
    int n3 = fx.length;
    int n2 = fx[0].length;
    int n1 = fx[0][0].length;
    int k1min=0, k2min=0, k3min=0;
    int k1max=n1-1, k2max=n2-1, k3max=n3-1;
    float fxmin = min(fx);
    float fkmin = fx[k3min][k2min][k1min];
    float fkmax = fx[k3max][k2max][k1max];
    for (int i3=0; i3<n3; ++i3) {
    for (int i2=0; i2<n2; ++i2) {
    for (int i1=0; i1<n1; ++i1) {
      float fxi = fx[i3][i2][i1];
      if(fxi==fkmin){fx[i3][i2][i1]=fxmin-5f;}
      if(fxi==fkmax){fx[i3][i2][i1]=fxmin-5f;}
    }}}
  }
  /**
