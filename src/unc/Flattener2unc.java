...
public class Flattener2unc {
    ...
    Sampling s1, Sampling s2, float[][] p2, float[][] el1, float[][] el2,float[][] w1) 
  {
    ...
    float wMin2 = 0.001f;
    float wMin1 = 0.001f;
    setWeightsForSmoothing(wMin1,wMin2,el1,el2);
    float[][] ws1 = mul(el1,2.0f);
    float[][] ws2 = mul(el2,2.0f);
    ...
    Smoother2 smoother2 = new Smoother2(n1,n2,_sigma1,_sigma2,ws1,ws2);
    A2 a2 = new A2(smoother2,w1,el1,p2);
  ...
  public void setWeights(float v1, float v2, float[][] w) {
    int n2 = w.length;
    int n1 = w[0].length;
    float[][] wo = copy(w);
    for (int i2=0; i2<n2; ++i2) {
      ...
      for (int i1=1; i1<n1; ++i1) {
        float wi = wo[i2][i1];
        float dwi = wi-wo[i2][i1-1];
        if(dwi<-0.2f){c++;}
        if(wi<0.001f){continue;}
        if(c==0) {w[i2][i1]=v1;}
        if(c==1) {w[i2][i1]=v2;}
        if(c==2) {w[i2][i1]=v1;}
      }
  ...
  private static float _wMin = 0.03f;
  ...
  private static void setWeightsForSmoothing(
    float wMin1, float wMin2, float[][] w1, float[][] w2) {
    int n2 = w1.length;
    int n1 = w2[0].length;
    for (int i2=0; i2<n2; ++i2) {
      for (int i1=0; i1<n1; ++i1) {
        float w1i = w1[i2][i1];
        float w2i = w2[i2][i1];
        w1[i2][i1] = (w1i>wMin1)?w1i:wMin1;
        w2[i2][i1] = (w2i>wMin2)?w2i:wMin2;
      }
    }
  }
