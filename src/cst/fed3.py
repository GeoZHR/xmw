...
def goCurvature():
  ...
  lsf = LocalSlopeFinder(2,1)
  lsf.findSlopes(fx,p2,p3,ep)
  cp = zerofloat(n1,n2,n3)
  cn = zerofloat(n1,n2,n3)
  ctf = ColorTensorFilter(1,3)
  ctf.applyForCurvature(p2,p3,cp,cn)
  plot3(fx,cmin=-1.0,cmax=1.0)
  plot3(cp,cmin=min(cp),cmax=max(cp))
  plot3(cn,cmin=min(cn),cmax=max(cn))
def goHessianFilter():
  fx = readImage(fxfile)
  p2 = zerofloat(n1,n2,n3)
  p3 = zerofloat(n1,n2,n3)
  ep = zerofloat(n1,n2,n3)
  lsf = LocalSlopeFinder(1,1)
  lsf.findSlopes(fx,p2,p3,ep)
  eu = zerofloat(n1,n2,n3)
  ev = zerofloat(n1,n2,n3)
  ctf = ColorTensorFilter(2,2)
  ctf.apply(p2,p3,eu,ev)
  plot3(fx,cmin=-1.0,cmax=1.0)
  #eu = abs(eu)
  plot3(eu,cmin=min(eu)/2,cmax=max(eu)/2)
  plot3(ev,cmin=min(ev)/2,cmax=max(ev)/2)
def goColorTensor():
  fx = readImage(fxfile)
