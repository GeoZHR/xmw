/****************************************************************************
Copyright 2007, University of Texas at Austin and others.
...
/**
 * Local estimates of orientations of features in color images.
 * @author Xinming Wu, University of Texas at Austin
 * @version 2017.07.1
 */
public class ColorTensorFilter {

  /**
   * Constructs a filter with an isotropic Gaussian window.
   * @param sigma half-width of window; same for all dimensions.
   */
  public ColorTensorFilter(double sigma) {
   ...
   */
  public ColorTensorFilter(double sigma1, double sigma2) {
   ...
   */
  public ColorTensorFilter(double sigma1, double sigma2, double sigma3) {
  ...
  public void applyForCurvature(float[][][] p2, float[][][] p3,
    float[][][] cp, float[][][] cn) 
  {
    // Gradient.
    int n3 = p2.length;
    ...
    _rgfGradient2.apply010(p2,g22);
    _rgfGradient3.apply001(p2,g23);
    _rgfGradient2.apply010(p3,g32);
    _rgfGradient3.apply001(p3,g33);
    /*
    for (int i3=0; i3<n3; ++i3) {
    for (int i2=0; i2<n2; ++i2) {
      for (int i1=0; i1<n1; ++i1) {
        ...
        float a = 0.5f*g22i;
        float b = 0.5f*g33i;
        float c = 0.5f*(g23i+g32i);
        //float d = p2[i3][i2][i1];
        //float e = p3[i3][i2][i1];
        float abp = a+b;
        float abm = a-b;
        float abc = sqrt(abm*abm+c*c);
        cp[i3][i2][i1] = abp+abc;
        cn[i3][i2][i1] = abp-abc;
      }
    }}
  ...
  public void apply(float[][][] p2, float[][][] p3,
    float[][][] eu, float[][][] ev) 
  {
    // Gradient.
    int n3 = p2.length;
    ...
    for (int i2=0; i2<n2; ++i2) {
      for (int i1=0; i1<n1; ++i1) {
        float a = g22[i3][i2][i1];
        float b = g23[i3][i2][i1];
        float c = g32[i3][i2][i1];
        float d = g33[i3][i2][i1];
        float T = 0.5f*(a+d);
        float D = a*d-b*c;
        float td = T*T-D;
        td = max(td,0.0f);
    ...
    float[][][] ep, float[][][] el)
  {
    ...
    float[][][] g2 = (nt>1)?t[1]:new float[n3][n2][n1];
    float[][][] g3 = (nt>2)?t[2]:new float[n3][n2][n1];
    _rgfGradient1.apply100(x[0],g1);
    _rgfGradient2.apply010(x[0],g2);
    _rgfGradient3.apply001(x[0],g3);

    // Gradient products.
    float[][][] g11 = (nt>3)?t[3]:new float[n3][n2][n1];
    float[][][] g22 = (nt>4)?t[4]:new float[n3][n2][n1];
    float[][][] g33 = (nt>5)?t[5]:new float[n3][n2][n1];
    float[][][] g12 = (nt>6)?t[6]:new float[n3][n2][n1];
    float[][][] g13 = (nt>7)?t[7]:new float[n3][n2][n1];
    float[][][] g23 = (nt>8)?t[8]:new float[n3][n2][n1];
    computeGradientProducts(g1,g2,g3,g11,g12,g13,g22,g23,g33);
    zero(g1);
    zero(g2);
    zero(g3);
    _rgfGradient1.apply100(x[1],g1);
    _rgfGradient2.apply010(x[1],g2);
    _rgfGradient3.apply001(x[1],g3);
    accumulateGradientProducts(g1,g2,g3,g11,g12,g13,g22,g23,g33);
    zero(g1);
    zero(g2);
    zero(g3);
    _rgfGradient1.apply100(x[2],g1);
    _rgfGradient2.apply010(x[2],g2);
    _rgfGradient3.apply001(x[2],g3);
    accumulateGradientProducts(g1,g2,g3,g11,g12,g13,g22,g23,g33);
    // Smoothed gradient products comprise the structure tensor.
    if (_rgfSmoother1!=null || _rgfSmoother2!=null || _rgfSmoother3!=null) {
      float[][][] h = (nt>9)?t[9]:new float[n3][n2][n1];
      float[][][][] gs = {g11,g22,g33,g12,g13,g23};
      for (float[][][] g:gs) {
        if (_rgfSmoother1!=null) {
          _rgfSmoother1.apply0XX(g,h);
  ...
  private void accumulateGradientProducts(
    final float[][][] g1, final float[][][] g2, final float[][][] g3,
    final float[][][] g11, final float[][][] g12, final float[][][] g13,
    final float[][][] g22, final float[][][] g23, final float[][][] g33)
  {
