...
public class FaultCell implements Serializable {
  ...
  public float[] getFaultNormal() {
    return faultNormalVectorFromStrikeAndDip(fp,ft);
  }
  public float[] getFaultDipVector() {
    return faultDipVectorFromStrikeAndDip(fp,ft);
  }
  public float[] getFaultStrikeVector() {
    return faultStrikeVectorFromStrikeAndDip(fp,ft);
  }
  ...
  float v1,v2,v3,vs; // strike vector
  ...
  boolean used = false;
  int nlr; // number of left and right neighbors
  int nab; // number of above and below neighbors
  int id;
