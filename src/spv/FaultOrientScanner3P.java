 ...
 * @version 2016.08.02
 */
public class FaultOrientScanner3P {
   ...
   */
  public FaultOrientScanner3P(float sigmaPhi, float sigmaTheta) {
    _sigmaPhi = sigmaPhi;
   ...
   * @param sigmaTheta half-width for smoothing along dip of fault planes.
   */
  public FaultOrientScanner3P(float power, float sigmaPhi, float sigmaTheta) {
    _sigmaPhi = sigmaPhi;
    _sigmaTheta = sigmaTheta;
  }
  ...
  public float[][][] getRotationAngleMap(int n1, int n2, int n3) {
    int h1 = (int)floor(n1/2);
    int h2 = (int)floor(n2/2);
    int h3 = (int)floor(n3/2);
    int hm = max(h1,h2);
    int hr = round(sqrt(hm*hm+h3*h3));
    int nr = 2*hr+1;
  ...
  // rotate a 3D iamge around axis1
  public float[][][] rotate1(float theta, float[][][] rsc, float[][][] gx) {
    final int n3 = gx.length;
    final int n2 = gx[0].length;
    final int n1 = gx[0][0].length;
    final int[][][] map = rotationMap1(n2,n3,theta,rsc);
    final int m3 = map[0].length;
    final int m2 = map[0][0].length;
    final float[][][] gr = new float[m3][m2][n1];
    ...
    public void compute(int i3) {
      ...
      for (int i2=0; i2<m2; ++i2) {
        ...
        int p3 = map33[i2];
      for (int i1=0; i1<n1; ++i1) {
        gr3[i2][i1] = gx[p3][p2][i1];
      }}
    }});
    return gr;
  }

  public int[][][] rotationMap1(int n2, int n3, float theta, float[][][] rsc) {
    final int nr = rsc[0].length;
    final int h3 = (int)floor(n3/2);
    final int h2 = (int)floor(n2/2);
    final int ri = (nr-1)/2;
    int r31 = abs(round(h3*cos(theta)+h2*sin(theta)));
    int r21 = abs(round(h2*cos(theta)-h3*sin(theta)));
    int r22 = abs(round(h2*cos(theta)+h3*sin(theta)));
    int r32 = abs(round(h3*cos(theta)-h2*sin(theta)));
    int r3 = max(r31,r32);
    int r2 = max(r21,r22);
    final int m3 = r3*2+1;
    final int m2 = r2*2+1;
    ...
    final int[][] p2s = new int[m3][m2];
    final int[][] p3s = new int[m3][m2];
    Parallel.loop(m3,new Parallel.LoopInt() {
    public void compute(int i3) {
      int k3 = i3+ri-r3;
      float[] rr3 = rsc[0][k3];
      ...
      for (int i2=0; i2<m2; ++i2) {
        ...
        int p2 = round(x2);
        int p3 = round(x3);
        p2s[i3][i2] = min(max(p2,0),n2-1);
        p3s[i3][i2] = min(max(p3,0),n3-1);
      }
    }});
    return new int[][][]{p2s,p3s};
  }

  // unrotate a 3D iamge around axis1
  public float[][][] unrotate1(int n1, int n2, int n3, float theta, 
    float[][][] rsc, float[][][] gr) {
    final float[][][] gu = new float[n3][n2][n1];
    final int[][][] map = unrotationMap1(n2,n3,theta,rsc);
    final Parallel.Unsafe<int[][][]> mapu = 
      new Parallel.Unsafe<int[][][]>();
    Parallel.loop(n3,new Parallel.LoopInt() {
    public void compute(int i3) {
      int[][][] mapt = mapu.get();
      if(mapt==null) mapu.set(mapt=new int[2][n3][n2]);
      mapt = map;
      float[][] gu3 = gu[i3];
      for (int i2=0; i2<n2; ++i2) {
        int p2 = map[0][i3][i2];
  ...
  public float[][][][] unrotate1(int n1, int n2, int n3, float theta, 
    float[][][] rsc, float[][][][] gr) {
    final float[][][][] gu = new float[2][n3][n2][n1];
    final int[][][] map = unrotationMap1(n2,n3,theta,rsc);
    Parallel.loop(n3,new Parallel.LoopInt() {
    public void compute(int i3) {
      int[] map23 = map[0][i3];
      ...
      for (int i2=0; i2<n2; ++i2) {
        ...
        int p3 = map33[i2];
        for (int i1=0; i1<n1; ++i1) {
          gu03[i2][i1] = gr[0][p3][p2][i1];
          gu13[i2][i1] = gr[1][p3][p2][i1];
        }
      }
    }});
    return gu;
  ...
  // unrotate a 3D iamge around axis1
  public int[][][] unrotationMap1(int n2, int n3, float theta, 
    float[][][] rsc) {
    final int h3 = (int)floor(n3/2);
    final int h2 = (int)floor(n2/2);
    final int nr = rsc[0].length;
    ...
    final float st = sin(theta);
    final float ct = cos(theta);
    final int[][] p2s = new int[n3][n2];
    final int[][] p3s = new int[n3][n2];
    Parallel.loop(n3,new Parallel.LoopInt() {
    public void compute(int i3) {
      int k3 = i3+ri-h3;
      float[] rr3 = rsc[0][k3];
      ...
      for (int i2=0; i2<n2; ++i2) {
        ...
        int p2 = round(x2);
        int p3 = round(x3);
        p2s[i3][i2] = min(max(p2,0),m2-1);
        p3s[i3][i2] = min(max(p3,0),m3-1);
      }
    }});
    return new int[][][]{p2s,p3s};
  }
  ...
  public float[][][] rotate2(final float theta, final float[][][] gx) {
    int n3 = gx.length;
    int n2 = gx[0].length;
    int n1 = gx[0][0].length;
    final float[][][] rsc = getRotationAngleMap(n1,n2,n3);
    final int[][][] map = rotationMap2(n1,n3,theta,rsc);
    final int m3 = map[0].length;
    final int m1 = map[0][0].length;
    final float[][][] gr = new float[m3][n2][m1];
    final Parallel.Unsafe<int[][][]> mapu = 
      new Parallel.Unsafe<int[][][]>();
    Parallel.loop(n2,new Parallel.LoopInt() {
    public void compute(int i2) {
      int[][][] mapt = mapu.get();
      if(mapt==null) mapu.set(mapt=new int[2][m3][m1]);
      mapt = map;
      for (int i3=0; i3<m3; i3++) {
      for (int i1=0; i1<m1; i1++) {
        int p1 = map[0][i3][i1];
        int p3 = map[1][i3][i1];
        gr[i3][i2][i1] = gx[p3][i2][p1];
      }}
    }});
    return gr;
  }

  public float[][][] unrotate2(
    final int n1, int n2, int n3, final float theta, final float[][][] gr) {
    final int m3 = gr.length;
    final int m1 = gr[0].length;
    final float[][][] rsc = getRotationAngleMap(n1,n2,n3);
    final int[][][] map = unrotationMap2(n1,n3,theta,rsc);
    final float[][][] gu = new float[n3][n2][n1];
    final Parallel.Unsafe<int[][][]> mapu = 
      new Parallel.Unsafe<int[][][]>();
    Parallel.loop(n2,new Parallel.LoopInt() {
  ...
  // rotate a 2D crossline slice around axis2
  public void rotate2(float theta, 
    float[][] rr, float[][] sr, float[][] cr, float[][] gx, float[][] gr) {
    int nr = rr.length;
    int ri = (nr-1)/2;
    int n3 = gx.length;
    int n1 = gx[0].length;
    ...
    for (int i1=0; i1<m1; ++i1) {
      ...
      int p1 = round(x1);
      int p3 = round(x3);
      p1 = min(max(p1,1),n1-2);
      p3 = min(max(p3,0),n3-1);
      gr[i3][i1] = gx[p3][p1];//si.interpolate(n1,1.0,0.0,gx[p3],x1);//gx[p3][p1];
    }}
  }

  public int[][][] rotationMap2(
    int n1, int n3, float theta, float[][][] rsc) {
    int nr = rsc[0].length;
    int ri = (nr-1)/2;
    int h1 = (int)floor(n1/2);
    int h3 = (int)floor(n3/2);
    int r31 = abs(round(h3*cos(theta)+h1*sin(theta)));
    int r11 = abs(round(h1*cos(theta)-h3*sin(theta)));
    int r12 = abs(round(h1*cos(theta)+h3*sin(theta)));
    int r32 = abs(round(h3*cos(theta)-h1*sin(theta)));
    int r1 = max(r11,r12);
    int r3 = max(r31,r32);
    int m3 = r3*2+1;
    int m1 = r1*2+1;
    float st = sin(theta);
    float ct = cos(theta);
    int[][] p3s = new int[m3][m1];
    int[][] p1s = new int[m3][m1];
    Parallel.loop(m3,new Parallel.LoopInt() {
    public void compute(int i3) {
      int k3 = i3-r3+ri;
      float[] rr3 = rsc[0][k3];
      ...
      for (int i1=0; i1<m1; ++i1) {
        ...
        int p1 = round(x1);
        int p3 = round(x3);
        p1s[i3][i1] = min(max(p1,0),n1-1);;
        p3s[i3][i1] = min(max(p3,0),n3-1);;
      }
    }});
    return new int[][][]{p1s,p3s};
  }


  // unrotate a 2D crossline slice around axis2
  public float[][] unrotate2(int n1, int n3, float theta, 
    ...
    for (int k1=0; k1<n1; ++k1) {
      ...
      int p1 = round(x1);
      int p3 = round(x3);
      p1 = min(max(p1,0),m1-1);
      p3 = min(max(p3,0),m3-1);
      gu[k3][k1] = gr[p3][p1];
    }}
    return gu;
  }

  public int[][][] unrotationMap2(
    int n1, int n3, float theta, float[][][] rsc) {
    int h1 = (int)floor(n1/2);
    int h3 = (int)floor(n3/2);
    int nr = rsc[0].length;
    ...
    float st = sin(theta);
    float ct = cos(theta);
    int[][] p1s = new int[n3][n1]; 
    int[][] p3s = new int[n3][n1]; 
    Parallel.loop(n3,new Parallel.LoopInt() {
    public void compute(int i3) {
      int k3 = i3-h3+ri;
      float[] rr3 = rsc[0][k3];
      ...
      for (int i1=0; i1<n1; ++i1) {
        ...
        int p1 = round(x1);
        int p3 = round(x3);
        p1s[i3][i1] = min(max(p1,0),m1-1);
        p3s[i3][i1] = min(max(p3,0),m3-1);
      }
    }});
    return new int[][][] {p1s,p3s};
  }
  ...
  private float[][][][] scan(
    ...
    for (int ip=0; ip<np; ++ip) {
      ...
      float phir = toRadians(phi);
      float[][][] rfx = rotate1(phir,rsc,fx);
      smooth2(rfx);
      float[][][][] rfpt = scanTheta(thetaSampling,rfx);
      rfx = null; // enable gc to collect this large array
      final float[][][][] fpt = unrotate1(n1,n2,n3,phir,rsc,rfpt);
      final float[][][] fp = fpt[0];
      final float[][][] tp = fpt[1];
      rfpt = null; // enable gc to collect this large array
      Parallel.loop(n3,new Parallel.LoopInt() {
      public void compute(int i3) {
        for (int i2=0; i2<n2; ++i2) {
          float[] f32 = f[i3][i2];
  ...
  private float[][][][] scanThetaX(Sampling thetaSampling, final float[][][] sn) {
    final int n3 = sn.length;
    final int n2 = sn[0].length;
    final int n1 = sn[0][0].length;
    ...
    for (int it=0; it<nt; ++it) {
      ...
      float sigma = (float)_sigmaTheta*sin(toRadians(ti));
      float theta = toRadians(90f-ti);
      final float[][][] snr = rotate2(theta,sn);
      Parallel.loop(n2,new Parallel.LoopInt() {
      public void compute(int i2) {
        float[][] sn2 = extractSlice2(i2,snr);
        RecursiveExponentialFilter ref = makeRef(sigma);
        ref.apply1(sn2,sn2);
        restoreSlice2(i2,snr,sn2);
      }});
      final float[][][] snu = unrotate2(n1,n2,n3,theta,snr);
      Parallel.loop(n3,new Parallel.LoopInt() {
      public void compute(int i3) {
        for(int i2=0; i2<n2; ++i2) {
        for(int i1=0; i1<n1; ++i1) {
          float fi = f[i3][i2][i1];
          float st = snu[i3][i2][i1];
  ...
  private static class Rotator {
    ...
    float[][][] rotate(float[][][] p) {
      ...
      Parallel.loop(n3q,new Parallel.LoopInt() {
        public void compute(int i3) {
    ...
    float[][][] unrotate(float[][][] q) {
      ...
      Parallel.loop(n3p,new Parallel.LoopInt() {
        public void compute(int i3) {
