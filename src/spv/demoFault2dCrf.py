...
def goPathVoting():
  ...
  if not plotOnly:
    ft = readImage(ftfile)
    pt = readImage(ptfile)
    osv = OptimalPathVoter(20,30)
    osv.setStrainMax(0.3)
  ...
  plot(gx,fv,cmin=0.5,cmax=1.0,cmap=jetRamp(1.0),
        neareast=False,label="Voting score",png="fv")
...
def goFaultLikelihood():
  ...
  gx = readImage(gxfile)
  gx = FaultScanner2.taper(15,0,gx)
  fs = FaultScanner2(15)
