...
def goFaultLikelihood():
  ...
  #fl,ft = fs.scan(75,85,sig1,sig2,smooth,gx)
  flt,ftt = fs.thin([fl,ft])
  print "fl min =",min(fl)," max =",max(fl)
  print "ft min =",min(ft)," max =",max(ft)
  plot(gx,fl,cmin=0.5,cmax=1,cmap=jetRamp(1.0),neareast=False,
       label="Fault likelihood",png="fl")
  plot(gx,flt,cmin=0.5,cmax=1,cmap=jetRamp(1.0),neareast=True,
...
def goLinearity():
  ...
  dst = DstCoherence(est,20)
  dst.setEigenvalues(1,0.01)
  dst.applyForLinear(gx,el)
  writeImage(elfile,el)
  el = pow(el,8)
  plot(gx,sub(1,el),cmin=0.5,cmax=1.0,cmap=jetRamp(1.0),label="1-linearity",png="el")
...
def goPathVoting():
  ...
  if not plotOnly:
    ...
    fv,w1,w2 = osv.applyVoting(1,0.5,ft,pt)
  ...
  plot(gx,fv,cmin=0.5,cmax=1.0,cmap=jetRamp(1.0),label="Voting score",png="fv")
  plot(gx,fvt,cmin=0.5,cmax=1.0,cmap=jetRamp(1.0),
       neareast=True,label="Voting score",png="fvt")
