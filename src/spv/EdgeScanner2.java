...
/**
 * Computes image edges, by scanning over edge orientations.
 *
 * @author Xinming Wu, University of Texas at Austin
 * @version 2017.07.15
 */

public class EdgeScanner2 {
  /**
   * Constructs an edge scanner with specified parameters.
   * @param sigmaTheta half-width for smoothing along dip of edges.
   */
  public EdgeScanner2(double sigma1, double sigma2) {
  ...
  private float[][][] scanTheta(Sampling thetaSampling, float[][] g) {
    ...
    for (int it=0; it<nt; ++it) {
      ...
      float[][] s2 = unrotate(n1,n2,theta,rsc[0],rsc[1],rsc[2],gg);
      for (int i2=0; i2<n2; ++i2) {
      for (int i1=0; i1<n1; ++i1) {
        float st = s2[i2][i1];
        if (st>f[i2][i1]) {
