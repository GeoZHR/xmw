...
gxfile = "101087"
gxfile = "69020"
elfile = "el"+gxfile # picked path using Sergey's method
eltfile = "elt"+gxfile # picked path using Sergey's method
ettfile = "ett"+gxfile # picked path using Sergey's method
smfile = "sm"
pngDir = getPngDir()
pngDir = None
plotOnly = False
...
def goEdgeScan():
  ...
  gs = computeGradient(gx[0])
  es = EdgeScanner2(8,4)
  el,et = es.scanColorImage(0,180,gx)
  el = sub(el,min(el))
  el = div(el,max(el))
  ef = es.edgeLikeFit(2,el,et)
  ef = clip(0,1,ef)
  elt,ett = es.thin([el,et])
  plot(gx[0],cmap=ColorMap.GRAY)
  plot(gs,cmap=ColorMap.GRAY)
  plot(el,cmap=ColorMap.GRAY)
  plot(ef,cmap=ColorMap.GRAY)
  plot(elt,cmap=ColorMap.GRAY)
  writeImage(elfile,el)
  writeImage(eltfile,ef)
  writeImage(ettfile,et)
