...
def goLinearity():
  ...
  el = pow(el,8)
  plot(gx,sub(1,el),cmin=0.5,cmax=1.0,cmap=jetRamp(1.0),label="Linearity")

def goFaultOrientScan():
  gx = readImage2L(n1,n2,gxfile)
  el = readImage(elfile)
  fos = FaultOrientScanner2(8)
  fe,fp = fos.scanDip(30,45,el)
...
def goPathVoting():
  ...
  if not plotOnly:
    ft = readImage(ftfile)
    pt = readImage(ptfile)
    osv = OptimalPathVoter(30,15)
    osv.setStrainMax(0.25)
    osv.setSurfaceSmoothing(2)
    fv,w1,w2 = osv.applyVoting(4,0.1,ft,pt)
  ...
  plot(gx,fv, cmin=0.01,cmax=0.4,cmap=jetRamp(1.0),label="Path voting")
  plot(gx,fvt,cmin=0.01,cmax=0.4,cmap=jetFillExceptMin(1.0),neareast=True,label="Path voting")
...
def goFaultLikelihood():
  ...
  fl,ft = fs.scan(30,45,p2,gx)
  flt,ftt = fs.thin([fl,ft])
  print "fl min =",min(fl)," max =",max(fl)
  print "ft min =",min(ft)," max =",max(ft)
  plot(gx,flt,cmin=0.01,cmax=0.5,cmap=jetFillExceptMin(1.0),neareast=True,
       label="Fault Likelihood")
  plot(gx,abs(ftt),cmin=30,cmax=45,cmap=jetFillExceptMin(1.0),
