  ...
  for i2 in range(n2):
    for i1 in range(n1):
      ...
      if(fpi<0):
        fpi = fpi+180
      fp[i2][i1] = fpi
  plot(gx,fp,cmin=0,cmax=180,cmap=hueFill(1.0),png="fp")
def goPathVoting():
  gx = readImage(gxfile)
  ep = readImage(epfile)
  fe = readImage(fefile)
  ...
  plot(gx,cmap=ColorMap.GRAY,cmin=-2,cmax=2,png="seis")
  ep = pow(ep,4)
  plot(ep,cmin=0.1,cmax=1.0,cmap=ColorMap.GRAY,png="ep")
  #plot(pow(sub(1,fv),0.5),cmin=0.001,cmax=1.0,png="fv")
  plot(sub(1,ep),cmap=ColorMap.GRAY,cmin=0.3,cmax=1.0,png="epm")
  plot(fv,cmap=ColorMap.GRAY,cmin=0.3,cmax=1.0,png="fv")
  #plot(sub(1,fe),cmin=0.1,cmax=0.8)
  plot(fvt,cmap=ColorMap.GRAY,nearest=True,cmin=0.3,cmax=1.0,png="fvt")

def goSeeds():
  ep = readImage(epfile)
  ft = readImage(ftfile)
  pt = readImage(ptfile)
  osv = OptimalPathVoter(20,35)
  sds = osv.pickSeeds(3,0.5,ft,pt)
  ep = pow(ep,4)
  ep = sub(1,ep)
  fs = osv.seedToImage(0.5,n1,n2,sds,ep)
  for i2 in range(1,n2-1,1):
    for i1 in range(1,n1-1,1):
      ...
      if(fsi>0.001):
        ...
        fs[i2  ][i1-1] = fsi
        fs[i2-1][i1-1] = fsi
  plot(fs,cmap=ColorMap.GRAY,cmin=0.001,cmax=0.5,nearest=True,png="seed")
...
def showPathes():
  ...
  osv = OptimalPathVoter(20,35)
  osv.setStrainMax(0.4)
  osv.setPathSmoothing(4)
  sds = osv.pickSeeds(3,0.5,ft,pt)
  x1,x2 = osv.getPathes(ep,ft,sds)
  plot(ep,ps=[x1,x2],cmap=ColorMap.GRAY,
       cmin=0.3,cmax=1.0,png="pathes")
  sds1 = osv.pickSeeds(20,0.5,ft,pt)
  x1,x2 = osv.getPathes(ep,ft,sds1)
  plot(ep,ps=[x1,x2],cmap=ColorMap.GRAY,
       cmin=0.3,cmax=1.0,png="patheSub")

  np = len(x1)
  sdc = osv.getSeeds(318,329,ft,pt)
  osv = OptimalPathVoter(20,50)
  x1,x2 = osv.getPathes(ep,ft,sdc)
  fs = osv.seedToImage(0.5,n1,n2,sds,ep)
  for i2 in range(1,n2-1,1):
    for i1 in range(1,n1-1,1):
      fsi = fs[i2][i1]
      if(fsi>0.001):
        ...
        fs[i2  ][i1-1] = fsi
        fs[i2-1][i1-1] = fsi
  plot(fs,ps=[x1,x2],cmap=ColorMap.GRAY,wd=4,
       cmin=0.01,cmax=0.5,nearest=True,png="pathc")

  plot(ep,ps=[x1,x2],cmap=ColorMap.GRAY,wd=4,
       cmin=0.3,cmax=1.0,nearest=False,png="pathpc")
...
def plot(f,g=None,ps=None,t=None,cmap=ColorMap.JET,
        cmin=None,cmax=None,cint=None, wd=2, label=None,nearest=False,png=None): 
