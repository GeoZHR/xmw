...
public class SnakeMesh {
  ...
  public static class Node {
    ...
    /**
     * Compute the mean position of this node and its neighbor nodes.
     * @return array containing the {X,Y,Z} coordinates of the mean position.
     */
    public void meanPosition(float[] xm) {
      int k=0;
      xm[0] = xm[1] = xm[2] = 0.0f;
      FaceIterator fi = getFaces();
      while (fi.hasNext()) {
      ...
      while (fi.hasNext()) {
        Face face = fi.next();
        face.nodeA()._mark=false;
        face.nodeB()._mark=false;
        face.nodeC()._mark=false;
      }
    }
  ...
  public static class Face {
     ...
     */
    public float area() {
      return SnakeMesh.normalVector(_meshFace,(float[])null);
    }
    ...
    public float[] normalVector() {
      float[] vn = new float[3];
      SnakeMesh.normalVector(_meshFace,vn);
      return vn;
    }
    ...
    public float normalVector(float[] vn) {
      return SnakeMesh.normalVector(_meshFace,vn);
    }
  ...
  public TriangleGroup getTriangleGroup() {
      int nf = countFaces();
      FaceIterator fi = getFaces();
      float[]xyz = new float[9*nf];
      int k=0;
      while(fi.hasNext()) {
        Face fc = fi.next();
        Node na = fc.nodeA();
      ...
      return new TriangleGroup(true,xyz);
    }
