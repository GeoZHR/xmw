...
public class GetPoints {
  ...
  public float[][] getXf(int np, int k2, int k3, float[][][] g) {
    int n3 = g.length;
    int n2 = g[0].length;
    int n1 = g[0][0].length;
    ...
    for (int i2=0; i2<n2; ++i2) {
      int nk = 0;
      for (int i1=0; i1<n1; ++i1) {
        if(g[i3][i2][i1]>0.0f) {nk++;}
      }
      int sh = 90;
      if(nk>(np+sh)&&g[i3][i2][sh]>0.0f) {
        int k1 = 0;
        for (int i1=sh; i1<np+sh; i1++) {
          x1[k1] = k1;
          x2[k1] = k2;
          x3[k1] = k3;
          fx[k1] = g[i3][i2][i1];
