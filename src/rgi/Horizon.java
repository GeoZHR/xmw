...
public class Horizon {
  ...
  public float[][] xyzUvwRgb(
    Sampling s1, Sampling s2, Sampling s3, ColorMap cp, 
    float[] xyz, float[][][] f, float[][][] g) 
  {
    ...
    Sampling si1 = new Sampling(n1);
    Sampling si2 = new Sampling(n2);
    Sampling si3 = new Sampling(n3);
    float[] rgb = new float[nv*3];
    float[] uvw = new float[nv*3];
    float[][][] u1 = zerofloat(n1,n2,n3);
    float[][][] u2 = zerofloat(n1,n2,n3);
    float[][][] u3 = zerofloat(n1,n2,n3);
    ...
    for (int iv=0; iv<nv; iv++) {
      ...
      float z = xyz[ik+2];
      xyz[ik+2] += d1*2f;
      float k1 = (float)((z-f1)/d1);
      float k2 = (float)((y-f2)/d2);
      float k3 = (float)((x-f3)/d3);
      float c = valueAtPoint(k1,k2,k3,g);
      //si.interpolate(si1,si2,si3,g ,k1,k2,k3);
      float u = si.interpolate(si1,si2,si3,u1,k1,k2,k3);
      float v = si.interpolate(si1,si2,si3,u2,k1,k2,k3);
      float w = si.interpolate(si1,si2,si3,u3,k1,k2,k3);
      rgb[ik  ] = c;
      rgb[ik+1] = c;
      rgb[ik+2] = c;
      uvw[ik  ] = u;
    ...
    rgb=cp.getRgbFloats(rgb);
    return new float[][]{xyz,uvw,rgb};
  }

  public float valueAtPoint(float k1, float k2, float k3, float[][][] u) {
    int n1 = u[0][0].length;
    int i1 = (int)k1;
    int i2 = (int)k2;
    int i3 = (int)k3;
    ...
    if(i1p>=n1){i1p=n1-1;}
    float ui = u[i3][i2][i1];
    for (int i1k=i1m; i1k<=i1p; i1k++) {
      float uk=u[i3][i2][i1k];
      if(uk<ui) {ui=uk;}
    }
    return ui;
  }  
