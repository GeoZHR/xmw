...
public class FaultSlipConstraints {
  ...
  public FaultSlipConstraints(int n1, int n2, int n3, FaultSkin[] sks) {
    _sks = sks;
  ...
  public short[][][] faultMarks(short[][][] fmark) {
    int n3 = fmark.length;
    int n2 = fmark[0].length;
    int n1 = fmark[0][0].length;
    computeUnfaultShifts(n1,n2,n3);
    setCells(n1,n2,n3);
    flNormalization();
    ArrayList<short[][]> cl = new ArrayList<short[][]>();
    for (FaultSkin skin:_sks) {
      for (FaultCell fc:skin) {
        float[] fx = fc.getX();
        float[] fs = fc.getS();
        float[] fh = add(fx,fs);
        short[] fp = new short[3];
        short[] hp = getHangPoint(n1,n2,n3,fh); 
        short i1i = (short)fc.getI1();
        short i2i = (short)fc.getI2();
        short i3i = (short)fc.getI3();
        short i2m = (short)bound(fc.getM2(),n2);
        short i3m = (short)bound(fc.getM3(),n3);
        fp[0] = i1i;
        fp[1] = i2m;
        fp[2] = i3m;
        fmark[i3i][i2i][i1i] = 1;
        if(hp!=null) {cl.add(new short[][]{fp,hp});}
      }
    }
    int ns = cl.size();
    System.out.println("sets of control points:"+ns);
    short[][][] fh = new short[2][3][ns];
    for (int is=0; is<ns; ++is) {
      short[][] ps = cl.get(is);
      fh[0][0][is] = ps[0][0];
      fh[0][1][is] = ps[0][1];
  ...
  private short[] getHangPoint(int n1, int n2, int n3, float[] fh) {
    short[] hp = new short[3];
    float h1 = fh[0];
    float h2 = fh[1];
    float h3 = fh[2];
    float ds = 1000f;
    FaultCell hcell = null;
    int i1 = bound(round(h1),n1); 
    for (int k2=-5; k2<=5; ++k2) {
    for (int k3=-5; k3<=5; ++k3) {
      int i2 = bound(round(h2+k2),n2); 
      int i3 = bound(round(h3+k3),n3); 
      FaultCell cell = _cells[i3][i2][i1];
      if (cell!=null) {
        float d2 = h2-cell.getX2();
        float d3 = h3-cell.getX3();
        float si = d2*d2+d3*d3;
        if(ds>si) {ds=si;hcell=cell;}
      }
    }}
    if (hcell!=null) {
      hp[0] = (short)bound(hcell.getI1(),n1);
      hp[1] = (short)bound(hcell.getP2(),n2);
      hp[2] = (short)bound(hcell.getP3(),n3);
      return hp;
    }
    return null;
  }
