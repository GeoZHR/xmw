 ...
 * Modified from Dave's BlendedGridder3X by Xinming Wu
 * @version 2015.06.30
 */
public class BlendedGridder3X implements Gridder3 {

  /**
   * Constructs a gridder for default tensors.
   */
  public BlendedGridder3X() {
    this(null);
  }
   ...
   */
  public BlendedGridder3X(float[] f, float[] x1, float[] x2, float[] x3) {
    this(null);
   ...
   */
  public BlendedGridder3X(Tensors3 tensors) {
    setTensors(tensors);
  }
  ...
  private void gridNearest(int nmark, float[][][] t, float[][][] p) {
    ...
    TimeMarker3W tm = new TimeMarker3W(n1,n2,n3,_tensors);
    tm.setFaults(_fmark,_ffoot,_fhang);
    sw.start();
    tm.apply(t,m);
    sw.stop();
    _tms = sw.time();
