...
public class FaultScanner {
  ...
  public float[][][][] scan(
    ...
    return scanX(phiSampling,thetaSampling,snd);
  ...
  public float[][][][] scan(
    ...
    float[][][][] snd = new float[2][n3][n2][n1];
    for (int i3=0; i3<n3; ++i3) {
    for (int i2=0; i2<n2; ++i2) {
    for (int i1=0; i1<n1; ++i1) {
      snd[0][i3][i2][i1] = max(sndf[0][i3][i2][i1],sndg[0][i3][i2][i1]);
      snd[1][i3][i2][i1] = max(sndf[1][i3][i2][i1],sndg[1][i3][i2][i1]);
    }}}
    Sampling sp = makePhiSampling(phiMin,phiMax);
    Sampling st = makeThetaSampling(thetaMin,thetaMax);
    //return semblanceFromNumDen(sn,sd);
    //return scan(sp,st,snd);
    return scanX(sp,st,snd);
  }

  public static float[][][] phaseShift(float ph, float[][][] fx) {
    int n3 = fx.length;
    int n2 = fx[0].length;
    ...
    Sampling phiSampling, Sampling thetaSampling, float[][][][] snd) 
  {
    ...
    for (int ip=0; ip<np; ++ip) {
      ...
      for (int it=0; it<nt; ++it) {
        ...
        float[][][] flr = scanTheta(ti,rsnd);
        float[][][] fln = r.unrotate(flr);
        float[] ws = faultNormalVectorFromStrikeAndDip(phi,ti);
        float w1 = ws[0];
        float w2 = ws[1];
        float w3 = ws[2];
        float w11 = w1*w1;
        ...
        for (int i2=0; i2<n2; ++i2) {
        for (int i1=0; i1<n1; ++i1) {
          float fli = fln[i3][i2][i1];
          float flt = fls[i3][i2][i1];
          if(fli>flt) {
            fls[i3][i2][i1]=fli;
            fps[i3][i2][i1]=phi;
            fts[i3][i2][i1]=ti;
    ...
    float tMax = (float)thetaSampling.getLast();
    float tMin = (float)thetaSampling.getFirst();
    float[][][][] fpt = solveEigenproblems(tMin,tMax,g11,g12,g13,g22,g23,g33);
    //float[][][][] fpt = computeDipStrike(tMin,tMax,fls);
    return new float[][][][]{fls,fpt[0],fpt[1]};
  }

  private float[][][][] computeDipStrike(
    final float tMin, final float tMax, final float[][][] fl)
  {
    ...
    public void compute(int i3) {
      ...
      for (int i1=0; i1<n1; ++i1) {
        ...
        float fti = faultDipFromNormalVector(u1i,u2i,u3i);
        float fpi = faultStrikeFromNormalVector(u1i,u2i,u3i);
        if(fti<tMin){fti=tMin;}
        if(fti>tMax){fti=tMax;}
        ft[i3][i2][i1] = fti;
        fp[i3][i2][i1] = fpi;
      }}
    }});
    ...
    final float[][][] g22, final float[][][] g23, final float[][][] g33)
  {
    ...
    public void compute(int i3) {
      ...
      for (int i1=0; i1<n1; ++i1) {
        ...
        float fti = faultDipFromNormalVector(u1i,u2i,u3i)-2;
        float fpi = faultStrikeFromNormalVector(u1i,u2i,u3i);
        if(fti<tMin){fti=tMin;}
        if(fti>tMax){fti=tMax;}
        ft[i3][i2][i1] = fti;
  ...
  private float[][][] scanTheta(final float ti, float[][][][] snd) {
    final int n1 = n1(snd), n2 = n2(snd), n3 = n3(snd);
    final float[][][][] ft = like(snd);
    final float[][][] sn = snd[0];
    final float[][][] sd = snd[1];
